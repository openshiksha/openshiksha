    ...
    <div className="rounded-xl border border-ink-100 bg-paper p-4">
      <div className="flex items-start justify-between gap-3">
        ...
        <Badge tone={cluster.student_count >= 3 ? 'attention' : 'neutral'} className="shrink-0">
          {t(cluster.student_count === 1 ? 'teacher.studentsCountOne' : 'teacher.studentsCountMany', {
            count: cluster.student_count,
          })}
        </Badge>
      </div>
      ...
      {cluster.sample_remediation_tip && (
        <p className="mt-2 text-xs text-ink-600">
          <span className="font-semibold">{t('teacher.tryInClass')}</span>{' '}
          {cluster.sample_remediation_tip}
        </p>
      )}

      <p className="mt-3 text-xs text-ink-400">
        {t(cluster.occurrence_count === 1 ? 'teacher.seenTimesOne' : 'teacher.seenTimesMany', {
          count: cluster.occurrence_count,
          last: formatDate(cluster.last_seen),
          updated: formatDate(cluster.refreshed_at),
        })}
      </p>
    </div>
  );
...
export const MisconceptionClustersPanel = ({ subjectRoomId }: Props) => {
  ...
  return (
    <div className="mt-3 border-t border-ink-100 pt-3">
      ...
      >
        <span>{t('teacher.misconceptions')}</span>
        ...
        <div className="mt-3 space-y-3">
          ...
          {!isLoading && isError && (
            <p className="text-xs text-rose-600 py-2">
              {t('teacher.misconceptionsLoadError')}{' '}
              <button
                type="button"
          ...
          {!isLoading && !isError && items.length === 0 && (
            <div className="text-xs text-ink-500 py-2">{t('teacher.noMisconceptions')}</div>
          )}

          {!isLoading && items.map((c) => <ClusterCard key={c.id} cluster={c} />)}

          {refresh.isError && (
            <p className="text-xs text-rose-600 pt-1">{t('teacher.refreshError')}</p>
          )}

          {refresh.isSuccess && (
            <p className="text-xs text-ink-400 pt-1" role="status">
              {t('teacher.recomputing')}
            </p>
          )}

          <div className="flex items-center justify-between pt-1">
            <p className="text-xs text-ink-400">{t('teacher.misconceptionsFootnote')}</p>
            <Button
              variant="ghost"
              size="sm"
            ...
            >
              {refresh.isPending ? t('teacher.refreshing') : t('teacher.refresh')}
            </Button>
          </div>
        </div>
      )}
