...
from openshiksha.apps.core.models import (
    Board, School, Standard, Subject, Chapter, ClassRoom,
    SubjectRoom, User, UserRole, Question, QuestionSubpart, QuestionTag,
    ProblemSet, Assignment, Submission,
)
from openshiksha.apps.edge.models import (
    Tick, StudentProficiency, SubjectRoomProficiency, SubjectRoomQuestionMistake,
)
...
def teacher(db, school):
    return User.objects.create_user(
        username='teacher_edge', password='pass', role=UserRole.TEACHER, school=school
    )
...
def student(db, school):
    return User.objects.create_user(
        username='student_edge', password='pass', role=UserRole.STUDENT, school=school
    )
...
def subpart(db, question):
    return QuestionSubpart.objects.create(
        question=question, index=0, correct_answer={'answer': 2}
    )
...
class TestStudentProficiency:
    def test_create_defaults(self, db, student, tag, subject_room):
        prof = StudentProficiency.objects.create(
    ...
    def test_apply_tick_accumulates(self, db, student, tag, subject_room, submission, subpart, subject_room_fixture=None):
        prof = StudentProficiency.objects.create(
            student=student, question_tag=tag, subject_room=subject_room
        )
        tick1 = Tick(student=student, question_subpart=subpart, submission=submission,
                     subject_room=subject_room, mark=1.0)
        tick1.save()
        tick2 = Tick(student=student, question_subpart=subpart, submission=submission,
                     subject_room=subject_room, mark=0.0)
        tick2.save()
...
class TestSubjectRoomProficiency:
    def test_create(self, db, tag, subject_room):
        room_prof = SubjectRoomProficiency.objects.create(
            question_tag=tag, subject_room=subject_room
        )
...
class TestSubjectRoomQuestionMistake:
    def test_create(self, db, subject_room, question):
        mistake = SubjectRoomQuestionMistake.objects.create(
            subject_room=subject_room, question=question
        )
