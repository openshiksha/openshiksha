...
|:--:|---|---|---|---|
| 1 | [Observability Activation](2026-observability-activation.md) | **Active — Batch 1 shipped** | **Promoted 2026-06-29.** Turns the validated-but-dead Production Observability artifacts into deployable, CI-gated k8s manifests. **Batch 1 (OACT-1..5) SHIPPED 2026-06-29** ([#488](https://github.com/openshiksha/openshiksha/pull/488)–[#491](https://github.com/openshiksha/openshiksha/pull/491) + docs PR): a self-contained `k8s/monitoring/` kustomization (Prometheus + ALT-1 rules → Alertmanager + `alert_to_ntfy.py` `python:3.13-slim` sidecar → Grafana provisioned with the MET-5 dashboard), all gated by an extended `k8s-manifests` job (kustomize + kubeconform `-strict`) and **not** wired into the auto-deploying prod overlay (operator applies with one `kubectl apply -k k8s/monitoring`); plus a two-phase prod backup pod wiring `record_backup_run` so `openshiksha_backup_age_seconds` reads real data. Runbook: [`docs/ops/monitoring-deploy.md`](../ops/monitoring-deploy.md). The third named follow-up (`VITE_SENTRY_DSN` in CI) is already done (`ci-cd.yaml:530`). All additive — qa/dev kustomize byte-for-byte unchanged, monitoring stack ignored by the prod overlay. | **Operator action:** `kubectl apply -k k8s/monitoring` on the droplet (resource-headroom call) + provision `METRICS_TOKEN`/`ALERT_NTFY_URL`/`GRAFANA_ADMIN_PASSWORD` + set `METRICS_ENABLED=true`. Then the North Star is reached — the next planning run promotes a fresh top initiative. |
