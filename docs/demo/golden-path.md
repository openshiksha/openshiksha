...
## Rehearsal status (DH-1 · last full pass: 2026-07-04)
...
|-------|----------|--------|
| Backend (Beats 0, 1, 4, 6, 8, 11, 12) | `pytest` — `test_widget_fields` · `test_widget_variables` · `test_algebra` · `test_problem_verifier` · `test_widget_authoring` · `test_step_hint` · `test_practice_problem` | ✅ 169/169 |
| Frontend units (Beats 2, 5, 6b, 7, 9, 10, 13) | `vitest` — the 12 beat suites listed in the Verify blocks below | ✅ 169/169 |
...
| Screenshots | all four `docs/demo/assets/*.png` regenerated by the e2es | ✅ fresh 2026-07-04 |

**Fixed this pass:** the Beat-3 spec was writing its screenshot to a gitignored
`e2e/__artifacts__/` dir instead of `docs/demo/assets/`, so the committed
`dtb4-describe-to-build.png` had silently gone stale (the playground UI had
visibly evolved since it was captured). The spec now writes the asset path the
other three e2es use, so "regenerated on every run" is true for all four again.
