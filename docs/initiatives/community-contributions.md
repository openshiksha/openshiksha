...
**Status:** ✅ Closed (opened 2026-07-05, closed 2026-07-09) · **Owner:** plan/execute routines

> **Closeout (2026-07-09):** both tracks are complete. **Track B** shipped the
> full content pipeline (CP-1..6): schema+validator → CI-gated `contrib/packs/`
> intake → `import_content_pack` staging → in-app review queue with real
> previews → approve-materializes-with-attribution, with the widget-*code*
> funnel fenced to normal PR review throughout. **Track A** shipped OSS-1..5:
> cold-clone audit fixes, issue/PR templates + labels + 5 seeded good-first-issues,
> the widgets SDK guide, verified README diagram + screenshots, and the
> dual-funnel community docs linked from CONTRIBUTING, the README, and the
> landing page. The North Star is met: an outsider can get productive, author
> content packs or widgets validated by construction, and submit through a
> pipeline where the maintainer approves in-app — nothing external reaches
> students unreviewed. Future community work (new tasks, external PR triage)
> continues under the routines without this initiative doc needing to reopen.
...
**Queue empty — initiative ✅ Closed (2026-07-09).** OSS-1..5 and CP-1..6 all
shipped; both tracks' DoD met (see the Status closeout note). No open tasks.
New community work is handled by the routines directly and does not require
reopening this doc.
...
| 2026-07-09 | **OSS-5 shipped → initiative ✅ Closed.** Community-docs discoverability: the CONTRIBUTING dual-funnel section already existed (CP-6), so this closed the two remaining links — the README "Contributing" section now names both funnels (content packs + widgets SDK) instead of pointing only at CONTRIBUTING.md, and the public **landing page** gained a footer "GitHub" link to the repo (new i18n key `home.footerGithub` in en+hi to satisfy the complete-locale parity guard; a `<a target="_blank" rel="noreferrer">` styled like the sibling footer links). With OSS-5 done, **Track A (OSS-1..5) and Track B (CP-1..6) are both complete** → flipped the initiative Status to Closed with a DoD note. Frontend verified: `npm run lint` + `tsc --noEmit` + Vitest (HomePage incl. new footer-link test) + `npm run build` green. | [#531](https://github.com/openshiksha/openshiksha/pull/531) | Stacked on #530. Chose a footer link over a hero CTA to keep the marketing page's conversion focus intact while still making "this is open source" discoverable. Initiative doc now stays closed; routines carry future community work. |
