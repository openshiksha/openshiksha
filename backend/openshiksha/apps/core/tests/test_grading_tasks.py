...
def teacher(db, school):
    ...
    return User.objects.create_user(username="teacher_grading", password="pass", role=UserRole.TEACHER, school=school)
...
def student(db, school):
    ...
    return User.objects.create_user(username="student_grading", password="pass", role=UserRole.STUDENT, school=school)
...
def subpart_a(db, question_mcq):
    ...
    return QuestionSubpart.objects.create(question=question_mcq, index=0, correct_answer={"answer": 2})
...
def subpart_b(db, question_mcq):
    ...
    return QuestionSubpart.objects.create(question=question_mcq, index=1, correct_answer={"answer": 4})
...
class TestGradeSubmissionTask:
    ...
    def test_grade_submission_creates_ticks(self, mock_prof, mock_agg, submission_with_answers, subpart_a, subpart_b):
    ...
    @patch("openshiksha.apps.core.tasks._update_assignment_aggregates")
    @patch("openshiksha.apps.core.tasks.update_proficiency")
    def test_grade_submission_score(self, mock_prof, mock_agg, submission_with_answers):
    ...
    @patch("openshiksha.apps.core.tasks._update_assignment_aggregates")
    @patch("openshiksha.apps.core.tasks.update_proficiency")
    def test_grade_submission_saves_score_to_db(self, mock_prof, mock_agg, submission_with_answers):
        from openshiksha.apps.core.tasks import grade_submission
