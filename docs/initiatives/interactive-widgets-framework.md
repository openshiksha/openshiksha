...
| 2026-06-05 | **IW-1a** — protocol types (`HostMessage = InitMessage`, `WidgetMessage = Ready \| Resize \| Value \| Error`) + narrow type guards + `WIDGET_PROTOCOL_VERSION` const; 10 vitest cases. | _this PR_ | Pure types; zero runtime risk; pins the wire contract IW-1b (host) and IW-1c (runtime) both import. Origin check is deliberately *not* in the protocol — that lives in `host.ts` because `allow-scripts` sandboxes have `event.origin === "null"`. |
