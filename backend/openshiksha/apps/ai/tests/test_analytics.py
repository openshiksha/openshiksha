...
def make_subject_room(db, classroom, subject, teacher, is_active=True):
    ...
    return SubjectRoom.objects.create(
        classroom=classroom, subject=subject, teacher=teacher, is_active=is_active
    )
...
def make_question(db, chapter, subject, standard):
    ...
    return Question.objects.create(
        chapter=chapter, subject=subject, standard=standard, question_type=QuestionType.MCQ
    )
...
def make_problem_set(db, school, standard, subject, chapter, teacher):
    ...
    return ProblemSet.objects.create(
        ...
        title='Test Set', created_by=teacher
    )
...
def base_objects(db):
    ...
    return {
        'student': student, 'teacher': teacher, 'subject_room': subject_room,
        'chapter': chapter, 'chapter2': chapter2,
        'subpart': subpart, 'subpart2': subpart2,
        'submission': submission,
    }
...
class TestDetectGapsForStudent:
    def test_no_ticks_returns_empty(self, base_objects):
        result = detect_gaps_for_student(
            base_objects['student'], base_objects['subject_room']
        )
        assert result == []
    ...
    def test_fewer_than_min_ticks_ignored(self, db, base_objects):
        ...
        for _ in range(MIN_TICKS_FOR_GAP - 1):
            make_tick(
                db, base_objects['student'], base_objects['subpart'],
                base_objects['submission'], base_objects['subject_room'], mark=0.2
            )
        result = detect_gaps_for_student(
            base_objects['student'], base_objects['subject_room']
        )
    ...
    def test_good_performance_marks_resolved(self, db, base_objects):
        for _ in range(MIN_TICKS_FOR_GAP):
            make_tick(
                db, base_objects['student'], base_objects['subpart'],
                base_objects['submission'], base_objects['subject_room'], mark=0.8
            )
        result = detect_gaps_for_student(
            base_objects['student'], base_objects['subject_room']
        )
    ...
    def test_borderline_score_not_flagged(self, db, base_objects):
        ...
        for _ in range(MIN_TICKS_FOR_GAP):
            make_tick(
                db, base_objects['student'], base_objects['subpart'],
                base_objects['submission'], base_objects['subject_room'],
...
class TestGenerateInsightsForSubjectRoom:
    ...
    def test_two_students_creates_insight(self, db, base_objects):
        ...
        for _ in range(MIN_TICKS_FOR_GAP):
            make_tick(
                db, student2, base_objects['subpart'],
                submission2, base_objects['subject_room'], mark=0.3
            )
...
class TestPredictPerformance:
    def test_insufficient_ticks_returns_none(self, base_objects):
        result = predict_performance_for_student(
            base_objects['student'], base_objects['subject_room']
        )
    ...
    def test_prediction_returned_with_enough_ticks(self, db, base_objects):
        for _ in range(MIN_TICKS_FOR_PREDICTION + 2):
            make_tick(
                db, base_objects['student'], base_objects['subpart'],
                base_objects['submission'], base_objects['subject_room'], mark=0.7
            )
        result = predict_performance_for_student(
            base_objects['student'], base_objects['subject_room']
        )
        ...
        assert result['readiness_level'] in [
            'needs_attention', 'developing', 'on_track', 'exam_ready'
        ]
