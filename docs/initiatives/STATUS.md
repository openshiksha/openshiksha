...
**Last updated:** 2026-06-23 (latest) — **Accessibility Batch 2 CLOSED — student
...
| 1 | [Accessibility — WCAG 2.1 AA](2026-accessibility-wcag-aa.md) | **Active** | **Batch 1 (A11Y-1..5) + Batch 2 (A11Y-4/FV/6/7/8) shipped.** Batch 1 ([#389](https://github.com/openshiksha/openshiksha/pull/389)–[#391](https://github.com/openshiksha/openshiksha/pull/391)): per-route axe baseline + static `eslint-plugin-jsx-a11y` gate; gated the public/auth surfaces. Batch 2 ([#417](https://github.com/openshiksha/openshiksha/pull/417)–[#420](https://github.com/openshiksha/openshiksha/pull/420) + this gate PR): brand-shade decision (`brand-700 → #C05300`, gated `/`), keyboard focus-visible pass, **authenticated axe harness** (`e2e/support/auth.ts`), and the **student core loop** (`/student`, assignment detail, proficiency, SRS drill) baselined → remediated (`StreakBadge` contrast) → **gated** (`blocking === []`). DoD items 1–4 done. | **Batch 3 — teacher/parent surfaces:** reuse the A11Y-6 authenticated harness (add teacher/parent users + their core reads) → baseline → remediate → gate. Then **Batch 4:** full keyboard-only + screen-reader (NVDA/VoiceOver) sign-off. |
