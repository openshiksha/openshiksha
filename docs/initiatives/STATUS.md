...
two-phase prod backup pod that populates `openshiksha_backup_age_seconds`
([#488](https://github.com/openshiksha/openshiksha/pull/488)–[#491](https://github.com/openshiksha/openshiksha/pull/491)
+ docs PR). Operator-applied (`kubectl apply -k k8s/monitoring`), **not** wired
into the auto-deploying prod overlay. Runbook:
[`docs/ops/monitoring-deploy.md`](../ops/monitoring-deploy.md). The only remaining
step to the North Star is the operator's deliberate apply on the droplet.
*(Promotion note below.)*

**Last updated:** 2026-06-29 — **NEW TOP INITIATIVE PROMOTED:
[Observability Activation](2026-observability-activation.md).** Production
...
Alertmanager are not yet deployed in `k8s/`"). Four batches built an instrument
panel **nobody has plugged in**: `/metrics` emits gauges nothing scrapes, eight
alert rules sit in a YAML file no Prometheus loads, a Grafana dashboard JSON has
no Grafana. With the board carrying **no other unblocked next bet** (OSS
Readiness, Accessibility, Mobile Shell, Language Access all Done; Interactive
Widgets Paused; LA-10 blocked; AI-Native fenced to the `ai-features` routine), the
2026-06-29 planning run promotes **Observability Activation** — package the
validated artifacts into a **deployable, CI-gated `k8s/monitoring/` kustomization**
(operator-applied, **not** wired into the auto-deploying prod overlay) and close
the one real code gap (the backup-freshness gauge prod never populates). **Batch 1
(OACT-1..5)** planned in [2026-06-29-plan.md](../daily-plans/2026-06-29-plan.md):
Prometheus manifest + CI kustomize/kubeconform gate (OACT-1) → Alertmanager +
`alert_to_ntfy.py` sidecar (OACT-2) → Grafana provisioned with datasource +
MET-5 dashboard (OACT-3) → wire `record_backup_run` into the nightly backup
CronJob (OACT-4) → `monitoring-deploy.md` runbook + close-out (OACT-5). The third
named follow-up (`VITE_SENTRY_DSN` in CI) is **already done** (`ci-cd.yaml:530`) —
only a DSN secret is pending. All additive: qa/dev kustomize byte-for-byte
unchanged, monitoring stack ignored by the prod overlay so nothing auto-deploys to
the live droplet. The `ai-features` routine fence is respected. *(Prior update
below.)*

**Last updated:** 2026-06-28 — **PRODUCTION OBSERVABILITY — INITIATIVE
DONE ✅ — North Star reached.** Batch 4 (Uptime & alerting, ALT-1..5) shipped
([#481](https://github.com/openshiksha/openshiksha/pull/481)–[#484](https://github.com/openshiksha/openshiksha/pull/484)
+ docs PR): a Prometheus alert-rules artifact (8 alerts on the metrics the platform
already emits) → an Alertmanager config + a dependency-light stdlib ntfy bridge
(`scripts/ops/alert_to_ntfy.py`) → a **standalone prod-overlay-only uptime probe
CronJob** that curls `/healthz/`+`/readyz/` and pushes to ntfy with **zero
Prometheus deployed** → an `alerting-lint` CI gate (`promtool`/`amtool` + bridge
tests) → `docs/ops/alerting.md` runbook + Sentry issue-alert routing. The
operability spine is complete: **observable → measurable → recoverable →
alerting.** All additive (qa/dev kustomize byte-for-byte unchanged, no secret
committed). **Honest follow-up:** Prometheus + Alertmanager are not yet deployed in
`k8s/` — ALT-1/2/4 are validated drop-in artifacts; the ALT-3 probe is the only
piece that alerts with zero stack. **No unblocked next bet remains on this board —
the next planning run promotes a fresh top initiative** (candidates: AI-tutor
rebase, an `/ai/predictions/` teacher surface, or un-pausing Interactive Widgets;
all subject to the `ai-features` fence for AI-Native). *(Prior update below.)*

**Last updated:** 2026-06-25 — **NEW TOP INITIATIVE PROMOTED:
...
|:--:|---|---|---|---|
| 1 | [Observability Activation](2026-observability-activation.md) | **Active — Batch 2 (durability & drift-safety) shipped** | **Promoted 2026-06-29.** Turns the validated-but-dead Production Observability artifacts into deployable, CI-gated k8s manifests. **Batch 1 (OACT-1..5) SHIPPED 2026-06-29** ([#488](https://github.com/openshiksha/openshiksha/pull/488)–[#491](https://github.com/openshiksha/openshiksha/pull/491) + docs PR): a self-contained `k8s/monitoring/` kustomization (Prometheus + ALT-1 rules → Alertmanager + `alert_to_ntfy.py` `python:3.13-slim` sidecar → Grafana provisioned with the MET-5 dashboard), all gated by an extended `k8s-manifests` job (kustomize + kubeconform `-strict`) and **not** wired into the auto-deploying prod overlay (operator applies with one `kubectl apply -k k8s/monitoring`); plus a two-phase prod backup pod wiring `record_backup_run` so `openshiksha_backup_age_seconds` reads real data. **Live-wiring follow-ups:** [#493](https://github.com/openshiksha/openshiksha/pull/493) (exempt `/metrics/` from HTTPS redirect) + [#494](https://github.com/openshiksha/openshiksha/pull/494) (route `openshiksha.org/grafana`). **Batch 2 (OACT-6..9, 11) SHIPPED 2026-06-30** ([#498](https://github.com/openshiksha/openshiksha/pull/498)–[#501](https://github.com/openshiksha/openshiksha/pull/501) + this docs PR) hardens the now-live stack: a manifest-vs-docs **parity guard** (drift fails CI), reconciled Grafana access docs, monitoring-stack **self-scrape** + `MonitoringTargetDown` alert, and an opt-in PVC-backed **persistent-storage overlay** (`k8s/monitoring-persistent`). Runbook: [`docs/ops/monitoring-deploy.md`](../ops/monitoring-deploy.md). All additive — qa/dev kustomize byte-for-byte unchanged, monitoring stack ignored by the prod overlay. | **Operator action (unchanged, last live step):** `kubectl apply -k k8s/monitoring` (or the new `k8s/monitoring-persistent` for retention) on the droplet (resource-headroom call) + provision `METRICS_TOKEN`/`ALERT_NTFY_URL`/`GRAFANA_ADMIN_PASSWORD` + set `METRICS_ENABLED=true`. Then the North Star is reached — the next planning run promotes a fresh top initiative. |
| - | [Production Observability & Operational Readiness](2026-production-observability.md) | **Done** | **Closed 2026-06-28 — North Star reached** (observable → measurable → recoverable → alerting). **Promoted 2026-06-25**; **Batch 1 (OBS-1..5) SHIPPED 2026-06-25** ([#460](https://github.com/openshiksha/openshiksha/pull/460)–[#464](https://github.com/openshiksha/openshiksha/pull/464)): `/readyz/` deep DB+cache probe + k8s readinessProbe repointed off the cheap `/healthz/` (drain-not-crash); `/api/v1/version/` build-info (git sha/build time baked via CI build-args); env-gated backend Sentry (`SENTRY_DSN`) + secret-scrubbing `before_send`; `RequestIDMiddleware` correlation id threaded through logs + `LOG_FORMAT=json` option; env-gated frontend error reporting (`VITE_SENTRY_DSN`) via the shared `ErrorBoundary` (dynamic import ⇒ entry chunk unchanged). All additive — no DSN / no `LOG_FORMAT=json` ⇒ behaviour byte-for-byte today's. **Batch 2 (MET-1..5) SHIPPED 2026-06-27** ([#467](https://github.com/openshiksha/openshiksha/pull/467)–[#471](https://github.com/openshiksha/openshiksha/pull/471)): gated Prometheus `/metrics` (`METRICS_ENABLED`/`METRICS_TOKEN`, 404 by default; `prometheus-client`, not always-on `django-prometheus`) → on-scrape business/queue-depth gauges (lazy-registered, fault-tolerant) → `TaskResult`-derived Celery health (gated on the `django-db` result backend; honest no-op under Redis) → gated HTTP request count + latency middleware → Grafana starter (`docs/ops/grafana/openshiksha-overview.json`) + scrape runbook (`docs/ops/metrics.md`). All additive — no `METRICS_ENABLED` ⇒ `/metrics` 404s, byte-for-byte today's. **Batch 3 (BAK-1..5) SHIPPED 2026-06-27** ([#474](https://github.com/openshiksha/openshiksha/pull/474)–[#477](https://github.com/openshiksha/openshiksha/pull/477) + docs PR): version-matched `pg_dump`/`pg_restore` scripts + `Dockerfile.backup` (`FROM postgres:15-alpine` + pinned static `mc`) → a *tested* local restore drill asserting row-count parity (`scripts/backup/drill.sh`, non-destructive scratch DB) → a prod-overlay-only nightly `CronJob` (`k8s/overlays/prod/backup-cronjob.yaml`; CI publishes the `openshiksha-backup` image) → a `BackupRun` freshness gauge (`openshiksha_backup_age_seconds`) on the MET-2 collector → `docs/ops/backups.md` runbook. All additive — qa/dev kustomize byte-for-byte unchanged, no secret committed. **Batch 4 (ALT-1..5) SHIPPED 2026-06-28** ([#481](https://github.com/openshiksha/openshiksha/pull/481)–[#484](https://github.com/openshiksha/openshiksha/pull/484) + docs PR): Prometheus alert-rules artifact (8 alerts on the existing metrics; `promtool`-validated, threshold unit-tests) → Alertmanager config + dependency-light stdlib ntfy bridge (`scripts/ops/alert_to_ntfy.py`, pure formatter, ntfy URL from env never logged) → standalone prod-overlay-only uptime probe CronJob (curls `/healthz/`+`/readyz/`, pushes to ntfy with **zero Prometheus deployed**; reuses the BAK-1 image via a `command:` override; qa byte-for-byte unchanged) → `alerting-lint` CI gate (`promtool check/test rules` + `amtool check-config` + bridge pytest, pinned binaries) → `docs/ops/alerting.md` runbook + Sentry issue-alert routing. **North Star reached: observable → measurable → recoverable → alerting.** | **No unblocked next bet — the next planning run promotes a fresh top initiative** (candidates: AI-tutor rebase `ai/2026-06-04-ai-tutor-chat`, an `/ai/predictions/` teacher surface, or un-pausing Interactive Widgets; all subject to the `ai-features` fence for AI-Native). **Operational follow-ups (not blocking close):** deploy Prometheus + Alertmanager in `k8s/` so ALT-1/2/4 go live (until then the ALT-3 probe is the active alarm); wire `VITE_SENTRY_DSN` into the CI frontend build once a DSN is provisioned; wire `record_backup_run` into the backup Job so prod populates the freshness gauge. |
