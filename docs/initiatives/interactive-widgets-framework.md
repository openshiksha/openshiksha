...
| 2026-06-05 | **IW-1b** — `_sdk/host.ts` module (`buildLegacySrcDoc`, `buildHostSrcDoc`, `createHostBridge`) + `InteractiveWidget.tsx` rewired to delegate srcdoc + bridge through the SDK; 14 new vitest cases. | _this PR_ | Legacy thermo path is byte-equivalent (legacy srcdoc string unchanged). New framework srcdoc bakes the `init` payload with `</script>` escaped to `<</script>` so authoring cannot break out of the boot script. Bridge enforces `event.source === iframe.contentWindow` *before* dispatching — the security boundary's primary identifier. |
