...
| DTB-5 | **Variable-aware generation (backend)**: with `allow_variables`, the AI may emit `{{var}}` bindings + validated `variable_constraints` so a generated widget is **per-student randomized** via croupier. Deterministic guardrail reconciles token↔constraint (drops unbacked tokens, validates ranges); typed-leaf substitution makes a numeric `{{var}}` land as a number. Reuses the existing croupier path; the grader is untouched. | ✅ Done (#391) |
| DTB-5b | **Surface randomization in the Describe-it UI**: an "each student gets different numbers" toggle on the prompt box that sets `allow_variables` and persists the returned `variable_constraints` on attach, so the wow is on-screen. | ⬜ |
