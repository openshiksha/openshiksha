...
| 1 | [Accessibility — WCAG 2.1 AA](2026-accessibility-wcag-aa.md) | **Active** | **Batch 1 + Batch 2 + Batch 3 shipped.** Batch 1 ([#389](https://github.com/openshiksha/openshiksha/pull/389)–[#391](https://github.com/openshiksha/openshiksha/pull/391)): per-route axe baseline + jsx-a11y gate; public/auth surfaces gated. Batch 2 ([#417](https://github.com/openshiksha/openshiksha/pull/417)–[#420](https://github.com/openshiksha/openshiksha/pull/420), [#441](https://github.com/openshiksha/openshiksha/pull/441)): brand-shade decision (`brand-700 → #C05300`, gated `/`), focus-visible pass, **authenticated axe harness**, **student core loop** baselined → remediated → gated. Batch 3 ([#442](https://github.com/openshiksha/openshiksha/pull/442), [#450](https://github.com/openshiksha/openshiksha/pull/450) + this gate PR): harness generalized to teacher/parent roles; `/teacher`, `/teacher/questions`, `/teacher/grading`, `/parent` baselined → `brand-on-tint` contrast fixed → **gated** (`blocking === []`). DoD items 1–4 done, item 6 partial. | **Finish Batch 3:** baseline + gate the dense authoring forms (`CreateQuestionPage`/`CreateAssignmentPage`/`CreateProblemSetPage`) + parent insights. Then **Batch 4:** full keyboard-only + screen-reader (NVDA/VoiceOver) sign-off (DoD item 5). |
