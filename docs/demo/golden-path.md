...
## Rehearsal status (DH-2 · last full pass: 2026-07-07)

All three phases are shipped, so the routine now **rehearses** the demo: every
beat's verification is re-run end to end and the reproducible screenshots are
regenerated, so a video shoot can trust that what the doc promises is what the
code does *today*.

| Layer | What ran | Result |
|-------|----------|--------|
| Backend (Beats 0, 1, 4, 6, 8, 11, 12, 15) | `pytest` — `test_widget_fields` · `test_widget_variables` · `test_algebra` · `test_problem_verifier` · `test_widget_authoring` · `test_step_hint` · `test_practice_problem` | ✅ 180/180 |
| Frontend units (Beats 2, 5, 6b, 7, 9, 10, 13, 16) | `vitest` — the 12 beat suites listed in the Verify blocks below | ✅ 172/172 |
| Real browser (Beats 3, 7, 10, 14) | `playwright` — `describe-to-build-grade` · `step-solver-grade` · `step-coach-autofeed` · `practice-problem-grade` | ✅ 7/7, no flakes |
| Screenshots | all four `docs/demo/assets/*.png` regenerated by the e2es | ✅ fresh 2026-07-07 |

**This pass (no drift):** triggered because the tree moved under the demo since
DH-1 — PV-5/PV-5b (Beats 15–16) landed after the last pass, and the community
CP-1/CP-3 increments touched `apps/core` (new model + migration). Everything
green with zero fixes needed; the suite counts grew to absorb the PV-5/PV-5b
tests (backend 169→180, frontend beat suites 169→172), and the refreshed
`pv4-practice-problem.png` now shows the Beat-16 randomization toggle in the
practice-problem card.

**Previous pass (DH-1, 2026-07-04) fixed:** the Beat-3 spec was writing its
screenshot to a gitignored `e2e/__artifacts__/` dir instead of
`docs/demo/assets/`, so the committed `dtb4-describe-to-build.png` had silently
gone stale. The spec now writes the asset path the other three e2es use, so
"regenerated on every run" is true for all four.
