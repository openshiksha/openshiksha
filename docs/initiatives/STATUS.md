...
|:--:|---|---|---|---|
| 1 | [Production Observability & Operational Readiness](2026-production-observability.md) | **Active** | **Promoted 2026-06-25**; **Batch 1 (OBS-1..5) SHIPPED 2026-06-25** ([#460](https://github.com/openshiksha/openshiksha/pull/460)–[#464](https://github.com/openshiksha/openshiksha/pull/464)): `/readyz/` deep DB+cache probe + k8s readinessProbe repointed off the cheap `/healthz/` (drain-not-crash); `/api/v1/version/` build-info (git sha/build time baked via CI build-args); env-gated backend Sentry (`SENTRY_DSN`) + secret-scrubbing `before_send`; `RequestIDMiddleware` correlation id threaded through logs + `LOG_FORMAT=json` option; env-gated frontend error reporting (`VITE_SENTRY_DSN`) via the shared `ErrorBoundary` (dynamic import ⇒ entry chunk unchanged). All additive — no DSN / no `LOG_FORMAT=json` ⇒ behaviour byte-for-byte today's. **Batch 2 (MET-1..5) SCOPED 2026-06-27** ([plan](../daily-plans/2026-06-27-plan.md)): gated Prometheus `/metrics` (`METRICS_ENABLED`/`METRICS_TOKEN`, 404 by default) → on-scrape business/queue-depth gauges → `TaskResult`-derived Celery health → gated HTTP request metrics middleware → Grafana starter + scrape runbook. | **Batch 2 — Metrics & dashboards** (MET-1..5, scoped — build next). Then Batch 3 — Backups & DR drill; Batch 4 — Uptime & alerting. **Follow-up:** wire `VITE_SENTRY_DSN` into the CI frontend build once a DSN is provisioned. |
