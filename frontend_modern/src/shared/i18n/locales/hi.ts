import type { LocaleDict } from './en';

/**
 * Hindi (हिंदी) locale. Loaded lazily — see I18nProvider — so the entry chunk
 * stays inside the performance budget.
 *
 * Register: everyday K-12 Hindi per the initiative Glossary
 * (docs/initiatives/2026-language-access.md). Classroom-English domain words
 * (Assignment, Dashboard, Score…) are transliterated, not academically
 * translated. `LocaleDict` enforces key parity with English at compile time;
 * the runtime parity test (LA-5) guards it in CI too.
 */
export const hi: LocaleDict = {
  // ── Common / switcher ────────────────────────────────────────────────
  'common.language': 'भाषा',
  'common.languageSwitchTo': 'भाषा बदलें: {language}',
  'common.cancel': 'रद्द करें',
  'common.practice': 'अभ्यास',
  'common.topicsOne': '{count} विषय',
  'common.topicsMany': '{count} विषय',

  // ── Auth hero (chalkboard panel, shared by login + register) ─────────
  'auth.heroHeadline': 'सीखने की कुंजी, एक-एक सवाल से।',
  'auth.heroSubtext':
    'आपकी गति से अभ्यास, तुरंत जाँच, और ऐसी जानकारी जो हर विद्यार्थी को बताए कि आगे क्या सीखना है।',
  'auth.heroFootnote': 'CBSE · कक्षा 7–10 · English & हिन्दी',

  // ── Login page ───────────────────────────────────────────────────────
  'login.title': 'वापसी पर स्वागत है',
  'login.subtitle': 'सीखना जारी रखने के लिए साइन इन करें।',
  'login.username': 'यूज़रनेम',
  'login.usernamePlaceholder': 'अपना यूज़रनेम लिखें',
  'login.password': 'पासवर्ड',
  'login.passwordPlaceholder': 'अपना पासवर्ड लिखें',
  'login.submit': 'साइन इन करें',
  'login.submitting': 'साइन इन हो रहा है…',
  'login.error': 'यूज़रनेम या पासवर्ड गलत है। कृपया फिर से कोशिश करें।',
  'login.noAccount': 'खाता नहीं है?',
  'login.registerLink': 'रजिस्टर करें',
  'login.schoolQuestion': 'क्या आप एक स्कूल हैं?',
  'login.enquireLink': 'OpenShiksha के बारे में पूछें',

  // ── Home (marketing) page ────────────────────────────────────────────
  'home.login': 'लॉग इन करें',
  'home.loginArrow': 'लॉग इन करें →',
  'home.heroTitle': 'हर बच्चे की क्षमता को खोलिए।',
  'home.heroSubtitle':
    'गणित और विज्ञान के लिए अनुकूली शिक्षा और शैक्षिक एनालिटिक्स — विद्यार्थियों के लिए मुफ़्त, और उनका मार्गदर्शन करने वाले शिक्षकों, अभिभावकों और स्कूलों के लिए बनाया गया।',
  'home.startFree': 'विद्यार्थी के रूप में मुफ़्त शुरू करें',
  'home.pillarPracticeTitle': 'अभ्यास',
  'home.pillarPracticeDesc':
    'गणित और विज्ञान में असीमित, अपने-आप बनने वाले प्रश्न — किन्हीं दो विद्यार्थियों को एक जैसा पेपर नहीं मिलता।',
  'home.pillarEvaluateTitle': 'जाँच',
  'home.pillarEvaluateDesc':
    'हर उत्तर की जाँच अपने-आप और तुरंत होती है — शिक्षकों के लिए दोहराव वाली कॉपी-जाँच नहीं।',
  'home.pillarAnalyseTitle': 'विश्लेषण',
  'home.pillarAnalyseDesc':
    'उन्नत एनालिटिक्स हर बच्चे की मज़बूतियाँ, कमियाँ और आगे क्या अभ्यास करना है, सामने लाती है।',
  'home.missionKicker': 'हमारा मिशन',
  'home.missionTitle':
    'गणित और विज्ञान का अभ्यास हर विद्यार्थी के लिए सचमुच रोचक बनाना।',
  'home.missionBody1':
    'OpenShiksha एक लर्निंग प्लेटफ़ॉर्म है जो अवधारणाओं की पकड़ और सीखने के परिणाम बेहतर करने के लिए बना है। हम कक्षा 7–10 का गणित और विज्ञान कवर करते हैं, कक्षा 1 से रिफ़्रेशर के साथ — सब CBSE बोर्ड के अनुरूप और English व हिंदी में उपलब्ध।',
  'home.missionOpenPre': 'हमारा',
  'home.missionOpenTerm': 'ओपन मॉडल',
  'home.missionOpenPost': 'किसी भी विद्यार्थी को मुफ़्त साइन अप करके सीखने देता है। हमारा',
  'home.missionPartnerTerm': 'पार्टनरशिप मॉडल',
  'home.missionPartnerPost':
    'स्कूलों और शैक्षिक संस्थाओं को वर्चुअल क्लासरूम चलाने और डेटा-आधारित निर्णय लेने के डैशबोर्ड देता है।',
  'home.teacherAlt': 'विद्यार्थियों की मदद करते हुए एक शिक्षक',
  'home.startNowTitle': 'अभी शुरू करें',
  'home.startNowSubtitle':
    'विद्यार्थियों के लिए हमेशा मुफ़्त। स्कूलों और शैक्षिक संस्थाओं के लिए मार्गदर्शित शुरुआत।',
  'home.studentsCardTitle': 'विद्यार्थी',
  'home.studentsCardDesc': 'मुफ़्त खाता बनाएँ और आज ही अभ्यास शुरू करें।',
  'home.studentsCardCta': 'मुफ़्त साइन अप करें →',
  'home.schoolsCardTitle': 'स्कूल और संस्थाएँ',
  'home.schoolsCardDesc': 'OpenShiksha को अपनी कक्षाओं तक लाएँ।',
  'home.schoolsCardCta': 'पूछताछ करें →',
  'home.featuresTitle': 'मुख्य विशेषताएँ',
  'home.feature1':
    'उन्नत एनालिटिक्स मज़बूतियाँ, कमज़ोरियाँ और ध्यान माँगती अवधारणाएँ ठीक-ठीक बताती है।',
  'home.feature2': 'व्यक्तिगत फ़ीडबैक हर विद्यार्थी के खास लर्निंग आउटकम पर केंद्रित होता है।',
  'home.feature3':
    'फ़ॉर्मूला-आधारित टेम्पलेट लगभग असीमित प्रश्न बनाते हैं — भरपूर अभ्यास, नकल नहीं।',
  'home.feature4': 'स्वचालित जाँच शिक्षकों की घंटों की दोहराव वाली कॉपी-जाँच हटा देती है।',
  'home.feature5':
    'साप्ताहिक AI सारांश वाला अभिभावक डैशबोर्ड परिवारों को जोड़े रखता है।',
  'home.feature6': 'कम कीमत के मोबाइल और टैबलेट पर भी बढ़िया चलता है — English और हिंदी में।',
  'home.footerSchools': 'स्कूलों के लिए',
  'home.footerGithub': 'GitHub',

  // ── Registration pages ───────────────────────────────────────────────
  'register.title': 'अपना खाता बनाएँ',
  'register.subtitle': 'आप कैसे सीखना चाहेंगे?',
  'register.haveAccount': 'पहले से खाता है?',
  'register.signIn': 'साइन इन करें',
  'register.back': '← वापस',
  'register.joinSchoolTitle': 'स्कूल से जुड़ें',
  'register.joinSchoolDesc':
    'अपने शिक्षक से मिला क्लासरूम जॉइन कोड इस्तेमाल करें — दाखिला अपने-आप हो जाएगा।',
  'register.openTitle': 'अपने दम पर पढ़ें',
  'register.openDesc':
    'साझा प्रश्न बैंक से अपनी गति से अभ्यास करें — स्कूल की ज़रूरत नहीं।',
  'register.openSubtitle': 'साझा प्रश्न बैंक मुफ़्त में इस्तेमाल करें।',
  'register.schoolTitle': 'अपने स्कूल से जुड़ें',
  'register.schoolSubtitle': 'अपने शिक्षक से मिला जॉइन कोड डालें।',
  'register.firstName': 'पहला नाम',
  'register.lastName': 'उपनाम',
  'register.username': 'यूज़रनेम',
  'register.password': 'पासवर्ड',
  'register.emailOptional': 'ईमेल (वैकल्पिक)',
  'register.joinCode': 'क्लासरूम जॉइन कोड',
  'register.joinCodePlaceholder': 'जैसे ABC123',
  'register.error': 'रजिस्ट्रेशन नहीं हो पाया। कृपया फिर कोशिश करें।',
  'register.creating': 'खाता बन रहा है…',
  'register.startPractising': 'अभ्यास शुरू करें',
  'register.createAccount': 'खाता बनाएँ',

  // ── Student dashboard ────────────────────────────────────────────────
  'dashboard.greeting': 'नमस्ते, {name}!',
  'dashboard.title': 'आपका डैशबोर्ड',
  'dashboard.subtitle': 'ये रहे आपके असाइनमेंट।',
  'dashboard.myProgress': 'मेरी प्रगति →',
  'dashboard.loadError': 'असाइनमेंट लोड नहीं हो पाए। कृपया पेज रीफ़्रेश करें।',
  'dashboard.openEmptyTitle': 'आप अभी किसी क्लासरूम में नहीं हैं',
  'dashboard.openEmptyDescription': 'अपने आप अभ्यास शुरू करने के लिए साझा प्रश्न बैंक देखें।',
  'dashboard.browseSubjects': 'विषय देखें →',

  // ── Assignment list (sections + empty state) ─────────────────────────
  'assignments.emptyTitle': 'अभी कोई असाइनमेंट नहीं',
  'assignments.emptyDescription': 'आपके शिक्षक जल्द ही असाइनमेंट देंगे। बाद में फिर देखें!',
  'assignments.sectionOverdue': 'समय निकल गया',
  'assignments.sectionDueSoon': 'जल्द जमा करें',
  'assignments.sectionUpcoming': 'आने वाले',
  'assignments.sectionCompleted': 'पूरे हो गए',

  // ── Assignment card ──────────────────────────────────────────────────
  'assignment.overdueBy': '{distance} देर हो चुकी',
  'assignment.dueIn': 'अंतिम तिथि: {distance}',
  'assignment.submittedAgo': '{distance} जमा किया',
  'assignment.submitted': 'जमा हो गया',
  'assignment.remedialBadge': 'सुधार अभ्यास',
  'assignment.progress': 'प्रगति',
  'assignment.ctaReview': 'देखें',
  'assignment.ctaContinue': 'जारी रखें',
  'assignment.ctaStart': 'शुरू करें',

  // ── Assignment detail (work + submit flow) ───────────────────────────
  'assignmentDetail.back': 'असाइनमेंट पर वापस जाएँ',
  'assignmentDetail.answeredCount': '{total} में से {answered} उत्तर दिए',
  'assignmentDetail.submittedNice': 'असाइनमेंट जमा हो गया — शाबाश!',
  'assignmentDetail.submittedTitle': 'जमा हो गया',
  'assignmentDetail.grading': 'जाँच चल रही है…',
  'assignmentDetail.submittedOfflineTitle': 'जमा हो गया',
  'assignmentDetail.gradePending': 'ऑनलाइन होने पर जाँच की जाएगी',
  'assignmentDetail.notFoundTitle': 'असाइनमेंट नहीं मिला',
  'assignmentDetail.notFoundDescription':
    'यह हटाया जा चुका हो सकता है या आपके पास इसकी अनुमति नहीं है।',
  'assignmentDetail.backToDashboard': 'डैशबोर्ड पर वापस जाएँ',
  'assignmentDetail.noQuestions': 'इस असाइनमेंट में कोई प्रश्न नहीं है',
  'assignmentDetail.submit': 'असाइनमेंट जमा करें',
  'assignmentDetail.confirmTitle': 'असाइनमेंट जमा करें?',
  'assignmentDetail.confirmBody':
    'आपने {total} में से {answered} प्रश्नों के उत्तर दिए हैं। जमा करने के बाद आप उत्तर नहीं बदल पाएँगे।',
  'assignmentDetail.confirmSubmit': 'जमा करें',
  'assignmentDetail.submitting': 'जमा हो रहा है…',

  // ── Due for Review panel (SRS) ───────────────────────────────────────
  'dueReview.title': 'दोहराव के लिए तैयार',
  'dueReview.overdueCount': '{count} का समय निकल गया',
  'dueReview.statusOverdue': 'समय निकल गया',
  'dueReview.statusToday': 'आज करना है',
  'dueReview.statusSoon': 'आगे आने वाला',
  'dueReview.overdueSince': '{date} से बाकी',
  'dueReview.dueOn': 'अंतिम तिथि {date}',
  'dueReview.interval': 'हर {days} दिन',
  'dueReview.reviewedTimes': '{count} बार दोहराया',
  'dueReview.moreTopics': '+{count} और विषय',
  'dueReview.emptyDescription':
    'अभी कुछ बाकी नहीं — कुछ असाइनमेंट पूरे करें, आपका दोहराव कार्यक्रम यहाँ दिखेगा।',
  'dueReview.footer': 'दोहराव की तिथियाँ आगे बढ़ाने के लिए अपने असाइनमेंट का अभ्यास करें।',

  // ── Recommendations panel ────────────────────────────────────────────
  'recommendations.title': 'आगे किसका अभ्यास करें',
  'recommendations.todaysPlan': 'आज की योजना: ~{minutes} मिनट',
  'recommendations.yourScore': 'आपका स्कोर',
  'recommendations.planTopicsOne': 'आज की योजना में {count} विषय',
  'recommendations.planTopicsMany': 'आज की योजना में {count} विषय',
  'recommendations.viewProgress': 'प्रगति देखें →',
  'recommendations.viewLearningPath': 'लर्निंग पाथ देखें →',
  'recommendations.emptyDescription':
    'अभी कोई सुझाव नहीं — कुछ असाइनमेंट प्रश्नों के उत्तर दें, हम आपको दोहराने लायक अध्याय बताएँगे।',

  // ── Explanation panel (AI) ───────────────────────────────────────────
  'explanation.cta': '✨ इस उत्तर को समझाएँ',
  'explanation.whyRight': 'यह उत्तर सही क्यों है',
  'explanation.whereWrong': 'कहाँ गलती हुई',
  'explanation.writing': 'आपकी व्याख्या लिखी जा रही है…',
  'explanation.error': 'अभी व्याख्या नहीं मिल पाई। कृपया थोड़ी देर में फिर कोशिश करें।',
  'explanation.retry': 'फिर कोशिश करें',
  'explanation.stubNote':
    'बिना AI मॉडल के बनाई गई — AI जुड़ने पर व्याख्याएँ और बेहतर होंगी।',
  'explanation.regenerateInLocale': 'हिंदी में समझाएँ',
  'explanation.regenerating': 'फिर से लिखी जा रही है…',

  // ── Parent dashboard ─────────────────────────────────────────────────
  'parent.title': 'अभिभावक डैशबोर्ड',
  'parent.description': 'अपने बच्चों की पढ़ाई की प्रगति देखें।',
  'parent.noChildrenTitle': 'आपके खाते से कोई बच्चा नहीं जुड़ा है',
  'parent.noChildrenDescription': 'अपने बच्चों के खाते जोड़ने के लिए स्कूल एडमिन से कहें।',
  'parent.gradeShort': 'कक्षा {grade}',
  'parent.grade': 'कक्षा {grade}',
  'parent.overview': '{name} की प्रगति-झलक',
  'parent.viewInsights': 'इनसाइट्स देखें →',
  'parent.tabProgress': 'प्रगति',
  'parent.tabAssignments': 'असाइनमेंट',
  'parent.noProgressTitle': 'अभी कोई प्रगति नहीं',
  'parent.noProgressDescription': '{name} ने अभी तक कोई असाइनमेंट जमा नहीं किया है।',
  'parent.noAssignmentsDescription': 'शिक्षक के असाइनमेंट बनाते ही वे यहाँ दिखेंगे।',
  'parent.questionsPractisedOne': '{count} प्रश्न का अभ्यास हुआ',
  'parent.questionsPractisedMany': '{count} प्रश्नों का अभ्यास हुआ',
  'parent.statusSubmitted': 'जमा हो गया',
  'parent.statusOverdue': 'समय निकल गया',
  'parent.statusPending': 'बाकी है',
  'parent.overdueOn': 'समय निकल गया — {date}',
  'parent.dueOn': 'अंतिम तिथि {date}',

  // ── Teacher dashboard + insight panels ───────────────────────────────
  'teacher.title': 'शिक्षक डैशबोर्ड',
  'teacher.subtitle': 'आपके रूम, प्रॉब्लम सेट और असाइनमेंट — सब एक जगह।',
  'teacher.aiGrading': '✨ AI जाँच',
  'teacher.newQuestion': '+ प्रश्न',
  'teacher.newProblemSet': '+ प्रॉब्लम सेट',
  'teacher.newAssignment': '+ असाइनमेंट',
  'teacher.statRooms': 'विषय रूम',
  'teacher.statStudents': 'विद्यार्थी',
  'teacher.statProblemSets': 'प्रॉब्लम सेट',
  'teacher.statOpenAssignments': 'खुले असाइनमेंट',
  'teacher.noRoomsTitle': 'अभी कोई विषय रूम नहीं',
  'teacher.noRoomsDescription': 'क्लासरूम से जुड़ने के लिए एडमिन से कहें।',
  'teacher.studentsCountOne': '{count} विद्यार्थी',
  'teacher.studentsCountMany': '{count} विद्यार्थी',
  'teacher.assign': 'असाइन करें',
  'teacher.viewInsights': 'क्लास इनसाइट्स देखें',
  'teacher.hideInsights': 'क्लास इनसाइट्स छिपाएँ',
  'teacher.newSetAction': '+ नया सेट',
  'teacher.noSetsTitle': 'अभी कोई प्रॉब्लम सेट नहीं',
  'teacher.noSetsDescription': 'प्रश्नों का ऐसा सेट बनाएँ जिसे आप किसी भी रूम को असाइन कर सकें।',
  'teacher.buildSet': 'प्रॉब्लम सेट बनाएँ',
  'teacher.questionsCountOne': '{count} प्रश्न',
  'teacher.questionsCountMany': '{count} प्रश्न',
  'teacher.minutesApprox': '~{minutes} मिनट',
  'teacher.previewAsStudent': 'विद्यार्थी की नज़र से देखें',
  'teacher.sectionJoinCodes': 'क्लास जॉइन कोड',
  'teacher.joinCodesDescription': 'कोड साझा करें ताकि विद्यार्थी खुद क्लासरूम से जुड़ सकें।',
  'teacher.sectionAssignments': 'असाइनमेंट',
  'teacher.noAssignmentsDescription': 'सबमिशन ट्रैक करने के लिए पहला असाइनमेंट बनाएँ।',
  'teacher.createAssignment': 'असाइनमेंट बनाएँ',
  'teacher.submittedRatio': '{submitted}/{total} जमा हुए',
  'teacher.avgScore': 'औसत: {pct}%',

  // Needs-attention strip
  'teacher.needsAttention': 'ध्यान दें · {count}',
  'teacher.bucketOverdue': '{count} समय पार',
  'teacher.bucketUngraded': '{count} बिना जाँचे',
  'teacher.bucketLowCompletion': '{count} कम सबमिशन',

  // Class health panel
  'teacher.classHealth': 'क्लास की सेहत',
  'teacher.loadingClassHealth': 'क्लास की सेहत का डेटा लोड हो रहा है…',
  'teacher.noClassHealth':
    'अभी क्लास की सेहत का डेटा नहीं। विद्यार्थियों के असाइनमेंट पूरे करने पर इनसाइट्स दिखेंगी।',
  'teacher.computing': 'गणना हो रही है…',
  'teacher.refresh': 'रीफ़्रेश करें',
  'teacher.refreshing': 'रीफ़्रेश हो रहा है…',
  'teacher.thChapter': 'अध्याय',
  'teacher.thAvg': 'औसत',
  'teacher.thStruggling': 'अटक रहे',
  'teacher.thStatus': 'स्थिति',
  'teacher.statusStruggling': 'अटक रहे',
  'teacher.statusAtRisk': 'जोखिम में',
  'teacher.statusProficient': 'निपुण',
  'teacher.lastUpdated': 'आख़िरी अपडेट: {datetime}',

  // Assignment detail page (RML-2)
  'teacher.adBackToDashboard': 'डैशबोर्ड पर वापस',
  'teacher.adLoadFailTitle': 'असाइनमेंट लोड नहीं हो सका',
  'teacher.adLoadFailDesc': 'शायद यह मौजूद नहीं है या आपके पास पहुँच नहीं है।',
  'teacher.adStatDue': 'नियत तिथि',
  'teacher.adStatSubmitted': 'जमा किया',
  'teacher.adStatAvg': 'औसत स्कोर',
  'teacher.adOverdue': 'समय बीत गया',
  'teacher.adActive': 'सक्रिय',
  'teacher.adSubmittedPct': '{pct}% जमा हुआ',
  'teacher.adSubmissionsTitle': 'छात्रों के सबमिशन',
  'teacher.adSubmissionsRecordedOne': '{count} सबमिशन दर्ज',
  'teacher.adSubmissionsRecordedMany': '{count} सबमिशन दर्ज',
  'teacher.adNotStarted': '{count} छात्रों ने शुरू नहीं किया',
  'teacher.adNoSubmissions': 'अभी कोई सबमिशन नहीं।',
  'teacher.adThStudent': 'छात्र',
  'teacher.adThSubmitted': 'जमा किया',
  'teacher.adThScore': 'स्कोर',
  'teacher.adThStatus': 'स्थिति',
  'teacher.adBadgeSubmitted': 'जमा किया',
  'teacher.adBadgePending': 'बाकी',
  'teacher.adGrading': 'जाँच हो रही है…',
  'teacher.adHardestTitle': 'सबसे कठिन प्रश्न',
  'teacher.adHardestSub': '(कुल खोए अंकों के अनुसार)',
  'teacher.adPtsLost': '{pts} अंक खोए',

  // Weekly AI summary panel
  'teacher.weeklySummary': 'साप्ताहिक AI समरी',
  'teacher.weeklyLoadError': 'अभी साप्ताहिक समरी लोड नहीं हो पाई।',
  'teacher.noWeeklySummary': 'अभी कोई साप्ताहिक समरी नहीं। इस हफ़्ते की अभ्यास गतिविधि से एक बनाएँ।',
  'teacher.generating': 'बन रही है…',
  'teacher.generate': 'बनाएँ',
  'teacher.weeklyGenError': 'अभी समरी नहीं बन पाई। कृपया थोड़ी देर में फिर कोशिश करें।',
  'teacher.weekOf': '{start} – {end} का सप्ताह',
  'teacher.stubSummaryNote': 'AI उपलब्ध नहीं था, इसलिए यह सीधे आपकी क्लास के डेटा से बनी है।',
  'teacher.practisedRatio': '{active}/{total} ने अभ्यास किया',
  'teacher.avgPct': '{pct}% औसत',
  'teacher.needsWork': 'मेहनत चाहिए:',
  'teacher.strong': 'मज़बूत:',
  'teacher.generatedOn': '{date} को बनी',
  'teacher.regenerating': 'फिर बन रही है…',
  'teacher.regenerate': 'फिर बनाएँ',

  // Interventions panel
  'teacher.interventions': 'मदद के सुझाव',
  'teacher.weakChaptersOne': '{count} कमज़ोर अध्याय',
  'teacher.weakChaptersMany': '{count} कमज़ोर अध्याय',
  'teacher.priority': 'प्राथमिकता {n}',
  'teacher.stubStrategyNote':
    'AI उपलब्ध नहीं था, इसलिए यह रणनीति सीधे {name} के लर्निंग-गैप डेटा से बनी है।',
  'teacher.recurringMisconception': 'दोहराई जा रही ग़लतफ़हमी:',
  'teacher.dismiss': 'हटाएँ',
  'teacher.markPlanned': 'योजना में जोड़ें',
  'teacher.planned': 'योजना में',
  'teacher.dismissed': 'हटाया गया',
  'teacher.resolve': 'पूरा हुआ',
  'teacher.interventionsLoadError': 'अभी सुझाव लोड नहीं हो पाए।',
  'teacher.noInterventions':
    'अभी कोई अटकता विद्यार्थी चिह्नित नहीं। मौजूदा लर्निंग-गैप डेटा से सुझाव बनाएँ।',
  'teacher.interventionsGenError': 'अभी सुझाव नहीं बन पाए। कृपया थोड़ी देर में फिर कोशिश करें।',
  'teacher.interventionsFootnote': 'AI रणनीतियाँ आपके फ़ॉलो-अप में मदद करती हैं — नियंत्रण आपके पास है।',

  // Misconception clusters panel
  'teacher.misconceptions': 'क्लास की ग़लतफ़हमियाँ',
  'teacher.tryInClass': 'क्लास में आज़माएँ:',
  'teacher.seenTimesOne': '{count} बार दिखी · आख़िरी बार {last} · अपडेट {updated}',
  'teacher.seenTimesMany': '{count} बार दिखी · आख़िरी बार {last} · अपडेट {updated}',
  'teacher.misconceptionsLoadError': 'अभी ग़लतफ़हमी पैटर्न लोड नहीं हो पाए।',
  'teacher.noMisconceptions':
    'अभी कोई ग़लतफ़हमी पैटर्न नहीं मिला — इस क्लास में कुछ असाइनमेंट जँचने के बाद वे दिखेंगे।',
  'teacher.refreshError': 'अभी रीफ़्रेश नहीं हो पाया। कृपया थोड़ी देर में फिर कोशिश करें।',
  'teacher.recomputing': 'हाल के सबमिशन से दोबारा गणना हो रही है — नए पैटर्न जल्द यहाँ दिखेंगे।',
  'teacher.misconceptionsFootnote': 'पूरी क्लास के पैटर्न — एक बार समझाएँ, सबकी मदद करें।',

  // AI assignment drafts panel
  'teacher.aiDrafts': 'AI असाइनमेंट ड्राफ़्ट',
  'teacher.draftPending': 'आपकी क्लास के कमज़ोर हिस्सों से ड्राफ़्ट बन रहा है…',
  'teacher.draftPendingNote': 'इसमें आमतौर पर कुछ सेकंड लगते हैं।',
  'teacher.draftFailed': 'यह ड्राफ़्ट नहीं बन पाया।',
  'teacher.retrying': 'फिर कोशिश हो रही है…',
  'teacher.tryAgain': 'फिर कोशिश करें',
  'teacher.practiceSetDefault': 'अभ्यास सेट',
  'teacher.assigned': 'असाइन हो गया',
  'teacher.viewAssignment': 'असाइनमेंट देखें',
  'teacher.classCanSee': 'आपकी क्लास इसे अब देख सकती है।',
  'teacher.stubDraftNote':
    'AI उपलब्ध नहीं था, इसलिए यह ड्राफ़्ट सीधे आपकी क्लास के अभ्यास डेटा से बना है।',
  'teacher.dismissDraftError': 'अभी ड्राफ़्ट नहीं हटा। कृपया थोड़ी देर में फिर कोशिश करें।',
  'teacher.approveEllipsis': 'स्वीकृत करें…',
  'teacher.dueDate': 'अंतिम तिथि',
  'teacher.titleLabel': 'शीर्षक',
  'teacher.titleOptionalHint': '(वैकल्पिक — खाली छोड़ने पर ड्राफ़्ट का शीर्षक रहेगा)',
  'teacher.approveError': 'अभी ड्राफ़्ट स्वीकृत नहीं हो पाया। कृपया थोड़ी देर में फिर कोशिश करें।',
  'teacher.useTryAgainHint': 'नया ड्राफ़्ट बनाने के लिए "फिर कोशिश करें" दबाएँ।',
  'teacher.assigning': 'असाइन हो रहा है…',
  'teacher.assignToClass': 'क्लास को असाइन करें',
  'teacher.draftsLoadError': 'अभी ड्राफ़्ट लोड नहीं हो पाए।',
  'teacher.noDrafts':
    'अभी कोई ड्राफ़्ट नहीं — AI को इस क्लास के सबसे कमज़ोर अध्यायों से अभ्यास सेट बनाने दें, फिर देखकर असाइन करें।',
  'teacher.draftStartError': 'अभी ड्राफ़्ट शुरू नहीं हो पाया। कृपया थोड़ी देर में फिर कोशिश करें।',
  'teacher.questionsLabel': 'प्रश्न',
  'teacher.difficultyLabel': 'कठिनाई',
  'teacher.starting': 'शुरू हो रहा है…',
  'teacher.draftAssignment': 'असाइनमेंट ड्राफ़्ट करें',
  'teacher.aiProposesFootnote':
    'AI सुझाता है, फ़ैसला आपका — आपकी स्वीकृति से पहले कुछ भी विद्यार्थियों तक नहीं पहुँचता।',

  // Question quality panel
  'teacher.questionQuality': 'प्रश्न गुणवत्ता',
  'teacher.flagTooHard': 'लगभग किसी ने सही नहीं किया — शब्दांकन या दर्ज सही उत्तर जाँचें।',
  'teacher.flagTooEasy': 'लगभग सबने सही किया — सेट में इसका योगदान कम है।',
  'teacher.flagMislabeled': 'यह अपने तय कठिनाई स्तर से ज़्यादा कठिन या आसान चल रहा है।',
  'teacher.flagLowDiscrimination':
    'तेज़ विद्यार्थियों ने भी बेहतर नहीं किया — अक्सर गलत दर्ज उत्तर का संकेत।',
  'teacher.flagOk': 'जैसा तय था वैसा ही चल रहा है।',
  'teacher.questionFallback': 'प्रश्न #{id}',
  'teacher.correctPct': '{pct} सही',
  'teacher.authoredDifficulty': 'तय कठिनाई',
  'teacher.observedDifficulty': 'देखी गई कठिनाई',
  'teacher.harderThanLabelled': 'लेबल से कठिन',
  'teacher.easierThanLabelled': 'लेबल से आसान',
  'teacher.discrimination': 'विभेदन',
  'teacher.calibrationSummaryOne': '{total} में से {flagged} कैलिब्रेटेड प्रश्न पर नज़र चाहिए।',
  'teacher.calibrationSummaryMany': '{total} में से {flagged} कैलिब्रेटेड प्रश्नों पर नज़र चाहिए।',
  'teacher.qualityLoadError': 'अभी प्रश्न विश्लेषण लोड नहीं हो पाया।',
  'teacher.noFlagged':
    'अभी कोई प्रश्न चिह्नित नहीं। कैलिब्रेशन के लिए हर प्रश्न पर कुछ प्रयास चाहिए — क्लास के अभ्यास के बाद रीफ़्रेश करें।',
  'teacher.recalibrateStartError': 'अभी रीकैलिब्रेशन शुरू नहीं हो पाया। कृपया थोड़ी देर में फिर कोशिश करें।',
  'teacher.recalibrating': 'हाल के उत्तरों से रीकैलिब्रेशन हो रहा है — नए नतीजे जल्द यहाँ दिखेंगे।',
  'teacher.qualityFootnote': 'नतीजे आपकी क्लास के असली उत्तरों से आते हैं — AI का अनुमान नहीं।',
  'teacher.calibrate': 'कैलिब्रेट करें',

  // Classroom join-code widget
  'teacher.joinCodeHeading': 'क्लासरूम जॉइन कोड',
  'teacher.copied': 'कॉपी हो गया!',
  'teacher.copyCode': 'कोड कॉपी करें',
  'teacher.copyLink': 'लिंक कॉपी करें',
  'teacher.noExpiry': 'कोई समय-सीमा नहीं',
  'teacher.expired': 'समय-सीमा बीत गई',
  'teacher.expiresInUnderHour': '1 घंटे से कम में खत्म',
  'teacher.expiresInHours': '{hours} घंटे में खत्म',
  'teacher.expiresInDays': '{days} दिन में खत्म',
  'teacher.regenerateWarning':
    'दोबारा बनाने पर मौजूदा कोड बेकार हो जाएगा। जो विद्यार्थी अभी नहीं जुड़े, उन्हें नया कोड चाहिए होगा।',
  'teacher.yesRegenerate': 'हाँ, दोबारा बनाएँ',
  'teacher.generateJoinCode': 'जॉइन कोड बनाएँ',

  // ── Create-assignment page ───────────────────────────────────────────
  'assignForm.eyebrow': 'रचना',
  'assignForm.title': 'असाइनमेंट बनाएँ',
  'assignForm.description':
    'अपने किसी विषय रूम को एक प्रॉब्लम सेट असाइन करें। विद्यार्थियों को यह तुरंत दिखेगा।',
  'assignForm.back': '← वापस',
  'assignForm.stepClass': 'क्लास चुनें',
  'assignForm.noRooms': 'आप अभी किसी विषय रूम को नहीं पढ़ाते।',
  'assignForm.subjectRoomLabel': 'विषय रूम',
  'assignForm.selectClass': 'कोई क्लास चुनें…',
  'assignForm.stepProblemSet': 'प्रॉब्लम सेट चुनें',
  'assignForm.chooseClassFirst': 'पहले एक क्लास चुनें।',
  'assignForm.noSetsForSubject': '{subject} के लिए अभी कोई प्रॉब्लम सेट नहीं।',
  'assignForm.buildOne': '+ एक बनाएँ',
  'assignForm.stepDueDate': 'अंतिम तिथि तय करें',
  'assignForm.dueDateLabel': 'अंतिम तिथि',
  'assignForm.quickSet': 'झटपट सेट:',
  'assignForm.preset3days': '3 दिन',
  'assignForm.preset1week': '1 सप्ताह',
  'assignForm.preset2weeks': '2 सप्ताह',
  'assignForm.dueTodayHint': 'आज देय · {date}',
  'assignForm.dueInDaysHintOne': '{count} दिन में देय · {date}',
  'assignForm.dueInDaysHintMany': '{count} दिन में देय · {date}',
  'assignForm.successTitle': 'असाइनमेंट बन गया!',
  'assignForm.successBodyOne':
    '{room} के {count} विद्यार्थी अब इसे अपने डैशबोर्ड पर देख सकते हैं। देय {date}।',
  'assignForm.successBodyMany':
    '{room} के {count} विद्यार्थी अब इसे अपने डैशबोर्ड पर देख सकते हैं। देय {date}।',
  'assignForm.backToDashboard': 'डैशबोर्ड पर वापस',
  'assignForm.createAnother': 'एक और बनाएँ',
  'assignForm.previewEmptyTitle': 'आपका असाइनमेंट प्रीव्यू',
  'assignForm.previewEmptyBody':
    'एक क्लास, एक प्रॉब्लम सेट और एक अंतिम तिथि चुनें — प्रकाशित करने से पहले आपको ठीक वही दिखेगा जो विद्यार्थियों को दिखेगा।',
  'assignForm.previewEyebrow': 'असाइनमेंट प्रीव्यू',
  'assignForm.pickProblemSet': 'एक प्रॉब्लम सेट चुनें',
  'assignForm.whatStudentsSee': 'विद्यार्थियों को क्या दिखेगा',
  'assignForm.pickSetLeft': 'बाईं ओर से एक प्रॉब्लम सेट चुनें।',
  'assignForm.previewQuestionsLink': 'विद्यार्थी की तरह असली प्रश्न देखें',
  'assignForm.dueLabel': 'देय',
  'assignForm.noDueYet': 'अभी कोई अंतिम तिथि तय नहीं',
  'assignForm.dueOn': 'देय {date}',
  'assignForm.dueOnTodaySuffix': 'देय {date} · आज',
  'assignForm.dueOnDaysSuffixOne': 'देय {date} · अब से {count} दिन',
  'assignForm.dueOnDaysSuffixMany': 'देय {date} · अब से {count} दिन',
  'assignForm.recipients': 'प्राप्तकर्ता',
  'assignForm.recipientsValueOne': '{room} के {count} विद्यार्थी',
  'assignForm.recipientsValueMany': '{room} के {count} विद्यार्थी',
  'assignForm.completeAllSteps': 'प्रकाशित करने के लिए तीनों चरण पूरे करें।',
  'assignForm.readyToPublishOne': '{count} विद्यार्थी को प्रकाशित करने के लिए तैयार।',
  'assignForm.readyToPublishMany': '{count} विद्यार्थियों को प्रकाशित करने के लिए तैयार।',
  'assignForm.createFailed': 'बनाने में विफल। कृपया फिर कोशिश करें।',
  'assignForm.publish': 'असाइनमेंट प्रकाशित करें',
  'assignForm.publishing': 'प्रकाशित हो रहा है…',

  // ── Build-problem-set page ───────────────────────────────────────────
  'setForm.title': 'प्रॉब्लम सेट बनाएँ',
  'setForm.description':
    'प्रश्नों को एक असाइन करने योग्य सेट में समूहित करें। किसी प्रश्न पर क्लिक करके देखें कि विद्यार्थियों को वह कैसा दिखेगा।',
  'setForm.details': 'विवरण',
  'setForm.titlePlaceholder': 'जैसे, द्विघात समीकरण अभ्यास सेट 1',
  'setForm.subject': 'विषय',
  'setForm.selectSubject': 'विषय चुनें…',
  'setForm.chapter': 'अध्याय',
  'setForm.selectChapter': 'अध्याय चुनें…',
  'setForm.chapterOption': '{name} (कक्षा {std})',
  'setForm.estimatedTime': 'अनुमानित समय (मिनट)',
  'setForm.estimatedPlaceholder': 'जैसे, 30',
  'setForm.selectQuestions': 'प्रश्न चुनें',
  'setForm.filteredOfTotal': '{total} में से {filtered}',
  'setForm.searchPlaceholder': 'प्रश्न का पाठ खोजें…',
  'setForm.difficultyFilter': 'कठिनाई:',
  'setForm.any': 'कोई भी',
  'setForm.typeFilter': 'प्रकार:',
  'setForm.selectSubjectToBrowse': 'प्रश्न देखने के लिए एक विषय चुनें।',
  'setForm.noQuestions': 'कोई प्रश्न नहीं मिला।',
  'setForm.authorOne': 'एक बनाएँ →',
  'setForm.noMatches': 'मौजूदा फ़िल्टर के लिए कोई मेल नहीं।',
  'setForm.previewEmptyTitle': 'प्रीव्यू के लिए एक प्रश्न चुनें',
  'setForm.previewEmptyDesc':
    'बाईं ओर की किसी पंक्ति पर क्लिक करके देखें कि विद्यार्थियों को यह कैसा दिखेगा — पूरा LaTeX, विकल्प, चित्र, सब कुछ।',
  'setForm.inSet': 'यह प्रश्न सेट में है।',
  'setForm.notInSet': 'अभी सेट में नहीं।',
  'setForm.removeFromSet': 'सेट से हटाएँ',
  'setForm.addToSet': '+ सेट में जोड़ें',
  'setForm.selectQuestionAria': 'प्रश्न {id} चुनें',
  'setForm.removeQuestionAria': 'प्रश्न {id} हटाएँ',
  'setForm.difficultyTitle': 'कठिनाई {n}/5',
  'setForm.partsCount': '{count} भाग',
  'setForm.interactive': 'इंटरैक्टिव',
  'setForm.questionsSelectedOne': 'प्रश्न चुना गया',
  'setForm.questionsSelectedMany': 'प्रश्न चुने गए',
  'setForm.successTitle': 'प्रॉब्लम सेट बन गया!',
  'setForm.successBodyOne': '{count} प्रश्न वाला {title} असाइन करने के लिए तैयार है।',
  'setForm.successBodyMany': '{count} प्रश्नों वाला {title} असाइन करने के लिए तैयार है।',
  'setForm.assignNow': 'अभी असाइन करें',
  'setForm.backToBank': 'प्रश्न बैंक पर वापस',
  'setForm.fillAllDesktop':
    'शीर्षक, विषय, अध्याय भरें और कम से कम एक प्रश्न चुनें।',
  'setForm.fillAllMobile': 'विषय, अध्याय, शीर्षक और कम से कम एक प्रश्न चुनें।',
  'setForm.create': 'प्रॉब्लम सेट बनाएँ',
  'setForm.createMobile': 'बनाएँ',
  'setForm.creating': 'बन रहा है…',

  // ── Open-response grading queue ──────────────────────────────────────
  'grading.title': 'AI जाँच',
  'grading.backDashboard': '← डैशबोर्ड',
  'grading.subtitle':
    'मुक्त उत्तर — AI जाँचता है, आप अंतिम रूप देते हैं। जब तक आप समीक्षा न करें, कुछ नहीं गिना जाता।',
  'grading.filterByClass': 'क्लास से फ़िल्टर करें',
  'grading.allMyClasses': 'मेरी सभी क्लास',
  'grading.roomOption': '{subject} · {classroom}',
  'grading.filterAll': 'सभी',
  'grading.filterNeedsReview': 'समीक्षा चाहिए',
  'grading.filterAiGrading': 'AI जाँच रहा',
  'grading.filterFinalised': 'अंतिम',
  'grading.filterFailed': 'विफल',
  'grading.statusPending': 'AI जाँच रहा…',
  'grading.statusAiGraded': 'आपकी समीक्षा चाहिए',
  'grading.statusReviewed': 'अंतिम',
  'grading.statusFailed': 'जाँच विफल',
  'grading.cardMeta': '{subject} · {date} को जमा',
  'grading.questionLabel': 'प्रश्न:',
  'grading.aiReading': 'AI इस उत्तर को पढ़ रहा है…',
  'grading.gradeFailed': 'इस उत्तर को जाँच नहीं सके।',
  'grading.retrying': 'फिर कोशिश हो रही है…',
  'grading.aiSuggests': 'AI सुझाव: {score}/{max}',
  'grading.confident': '{pct}% आश्वस्त',
  'grading.lowConfidenceNote':
    'AI इस उत्तर को लेकर अनिश्चित है — स्वीकार करने से पहले उत्तर पढ़कर दोबारा जाँच लें।',
  'grading.autoGraded': 'स्वतः जाँचा',
  'grading.stubNote':
    'AI उपलब्ध नहीं था, इसलिए यह सुझाव मॉडल उत्तर से कीवर्ड मिलान पर आधारित है — कृपया अतिरिक्त सावधानी से समीक्षा करें।',
  'grading.regrading': 'फिर जाँच हो रही है…',
  'grading.askAiAgain': 'AI से फिर पूछें',
  'grading.finalGrade': 'अंतिम अंक: {score}/{max}',
  'grading.aiSuggestedOverride': 'AI ने {score}/{max} सुझाया — आपने इसे बदला।',
  'grading.yourComment': 'आपकी टिप्पणी:',
  'grading.finalMarks': 'अंतिम अंक ({max} में से)',
  'grading.commentLabel': 'विद्यार्थी के लिए टिप्पणी',
  'grading.optional': '(वैकल्पिक)',
  'grading.commentPlaceholder': 'जैसे, अच्छा तर्क — अगली बार प्रक्रिया का नाम लिखें।',
  'grading.saveError': 'अभी अंक सहेज नहीं सके। कृपया थोड़ी देर में फिर कोशिश करें।',
  'grading.saving': 'सहेजा जा रहा है…',
  'grading.acceptSuggestion': 'सुझाव स्वीकारें',
  'grading.saveFinal': 'अंतिम अंक सहेजें',
  'grading.finaliseLocks': 'अंतिम रूप देने पर यह अंक लॉक हो जाता है।',
  'grading.loadError': 'अभी जाँच कतार लोड नहीं कर सके।',
  'grading.retry': 'फिर कोशिश करें',
  'grading.emptyFilteredTitle': 'इन फ़िल्टर से कुछ मेल नहीं खाता',
  'grading.emptyTitle': 'अभी जाँचने के लिए कोई उत्तर नहीं',
  'grading.emptyFilteredDesc': 'कोई दूसरी क्लास या स्थिति आज़माएँ।',
  'grading.emptyDesc':
    'किसी विद्यार्थी का मुक्त उत्तर दर्ज करें और AI आपकी समीक्षा के लिए एक अंक सुझाएगा।',

  // ── LA-6e-2: question-type labels (shared) ───────────────────────────
  'qtype.mcq': 'MCQ',
  'qtype.numeric': 'संख्यात्मक',
  'qtype.fill_blank': 'रिक्त स्थान',
  'qtype.multi_select': 'बहु-चयन',
  'qtype.matching': 'मिलान',
  'qtype.compound': 'संयुक्त',

  // ── LA-6e-2: question bank page ──────────────────────────────────────
  'qbank.eyebrow': 'लेखन',
  'qbank.title': 'प्रश्न बैंक',
  'qbank.description':
    'अपने असाइनमेंट में प्रश्न ब्राउज़ करें और दोबारा इस्तेमाल करें। किसी प्रश्न पर क्लिक करके देखें कि विद्यार्थी उसे ठीक कैसे देखेंगे।',
  'qbank.createQuestion': '+ प्रश्न बनाएँ',
  'qbank.searchPlaceholder': 'प्रश्न पाठ, विषय, अध्याय खोजें…',
  'qbank.allSubjects': 'सभी विषय',
  'qbank.anyDifficulty': 'कोई भी कठिनाई',
  'qbank.difficultyOption': '{stars} ({level}/5)',
  'qbank.allChapters': 'सभी अध्याय',
  'qbank.gradeSuffix': 'कक्षा {grade}',
  'qbank.pickSubjectFirst': 'पहले एक विषय चुनें',
  'qbank.clearFilters': 'फ़िल्टर हटाएँ',
  'qbank.noQuestionsFound': 'कोई प्रश्न नहीं मिला',
  'qbank.adjustFilters': 'फ़िल्टर बदलकर देखें।',
  'qbank.authorFirst': 'अपना पहला प्रश्न बनाएँ।',
  'qbank.createAQuestion': 'प्रश्न बनाएँ →',
  'qbank.resultsCountOne': '{count} प्रश्न',
  'qbank.resultsCountMany': '{count} प्रश्न',
  'qbank.difficultyTitle': 'कठिनाई {level}/5',
  'qbank.rowEdit': 'संपादित करें',
  'qbank.previewEmptyTitle': 'प्रीव्यू के लिए एक प्रश्न चुनें',
  'qbank.previewEmptyDesc':
    'बाईं ओर किसी पंक्ति पर क्लिक करके देखें कि विद्यार्थी उसे ठीक कैसे देखेंगे — पूरा LaTeX, विकल्प, चित्र, सब कुछ।',
  'qbank.reuseNote': 'इस प्रश्न को अपने किसी भी समस्या सेट में दोबारा इस्तेमाल करें।',
  'qbank.useInNewSet': 'नए सेट में इस्तेमाल करें',
  'qbank.addToProblemSet': '+ समस्या सेट में जोड़ें',

  // ── LA-6e-2: add-to-problem-set sheet ────────────────────────────────
  'addToSet.eyebrow': 'सेट में जोड़ें',
  'addToSet.questionTitle': 'प्रश्न #{id}',
  'addToSet.closeLabel': 'सेट-में-जोड़ें डायलॉग बंद करें',
  'addToSet.addedTitle': 'जोड़ा गया!',
  'addToSet.addedDesc': 'प्रश्न अब समस्या सेट में है।',
  'addToSet.done': 'पूरा',
  'addToSet.addAnother': 'किसी और में जोड़ें',
  'addToSet.noSets': '{subject} में अभी कोई समस्या सेट नहीं है।',
  'addToSet.thisSubject': 'इस विषय',
  'addToSet.buildOne': 'एक बनाएँ →',
  'addToSet.questionsCountOne': '{count} प्रश्न',
  'addToSet.questionsCountMany': '{count} प्रश्न',
  'addToSet.failedToAdd': 'जोड़ नहीं सके। कृपया फिर कोशिश करें।',
  'addToSet.add': 'जोड़ें',

  // ── LA-6e-2: question preview panel ──────────────────────────────────
  'qpreview.part': 'भाग {letter}',
  'qpreview.diagramAlt': 'प्रश्न का चित्र',
  'qpreview.numericAnswer': 'संख्यात्मक उत्तर',
  'qpreview.fillBlankResponse': 'रिक्त स्थान भरने वाला उत्तर।',
  'qpreview.interactiveWidget': '🧪 सैंडबॉक्स्ड इंटरैक्टिव विजेट (विद्यार्थियों के लिए रेंडर होता है)',
  'qpreview.emptyTitle': 'प्रीव्यू के लिए एक प्रश्न चुनें',
  'qpreview.emptyDesc':
    'बाईं ओर किसी पंक्ति पर क्लिक करके देखें कि विद्यार्थी उसे ठीक कैसे देखेंगे — पूरा LaTeX, विकल्प, चित्र, सब कुछ।',
  'qpreview.difficultyTitle': 'कठिनाई {level}/5',

  // ── LA-6e-2: widget gallery panel ────────────────────────────────────
  'widgetGallery.pickTitle': 'एक इंटरैक्टिव विजेट चुनें',
  'widgetGallery.pickDesc':
    'विजेट प्रश्न पाठ के साथ एक सैंडबॉक्स्ड iframe में रेंडर होता है। कॉन्फ़िगर करने के लिए एक चुनें — फ़ॉर्म भरते ही लाइव प्रीव्यू अपडेट होता है।',
  'widgetGallery.noneRegistered': 'अभी कोई शिक्षक-दृश्य विजेट पंजीकृत नहीं है।',
  'widgetGallery.answerBadge': 'उत्तर',
  'widgetGallery.noSchema': 'इस विजेट ने JSON Schema घोषित नहीं किया। नीचे रॉ कॉन्फ़िग संपादित करें।',
  'widgetGallery.backToGallery': '← गैलरी पर वापस',
  'widgetGallery.config': 'कॉन्फ़िग',
  'widgetGallery.preview': 'प्रीव्यू',
  'widgetGallery.previewNote':
    'हर कॉन्फ़िग बदलाव पर प्रीव्यू दोबारा रेंडर होता है। सैंडबॉक्स्ड iframe — विजेट के अंदर के स्क्रिप्ट पेज तक नहीं पहुँच सकते।',
  'widgetGallery.useThis': 'यह विजेट इस्तेमाल करें',

  // ── DTB-3: describe-to-build (AI widget authoring) ───────────────────
  'widgetGallery.describeTitle': 'इसे बताइए — AI विजेट बना देगा',
  'widgetGallery.describeDesc':
    'सरल भाषा में लिखिए कि आपको क्या चाहिए। AI एक मान्य, सैंडबॉक्स्ड विजेट सुझाएगा जिसे आप प्रीव्यू, संपादित और संलग्न कर सकते हैं — ग्रेडिंग निर्धारक ही रहती है।',
  'widgetGallery.describePlaceholder': 'जैसे: एक संख्या रेखा जहाँ छात्र 3/4 अंकित करें',
  'widgetGallery.describeButton': 'विजेट बनाएँ',
  'widgetGallery.describeGenerating': 'आपका विजेट बन रहा है…',
  'widgetGallery.describeError':
    'विजेट बिल्डर तक नहीं पहुँच सके। फिर से प्रयास करें, या नीचे से एक विजेट चुनें।',
  'widgetGallery.describeFallback':
    'AI अभी उपलब्ध नहीं है — यहाँ एक सुरक्षित शुरुआती विजेट है जिसे आप संपादित कर सकते हैं।',
  'widgetGallery.describeOrPick': 'या स्वयं एक चुनें',
  'widgetGallery.aiStubLabel': 'स्वतः-निर्मित',
  // ── DTB-5b: प्रति-छात्र यादृच्छिकीकरण ───────────────────────────────
  'widgetGallery.randomizeLabel': 'हर छात्र को अलग संख्याएँ मिलें',
  'widgetGallery.randomizeHint':
    'AI मानों को प्रति-छात्र चर से जोड़ता है ताकि हर छात्र को एक नया प्रश्न दिखे। मूल्यांकन नियतात्मक रहता है।',
  'widgetGallery.randomizeBadge': '🎲 प्रति छात्र यादृच्छिक',
  'widgetGallery.randomizeNote': 'हर छात्र को इनके लिए नए मान मिलते हैं:',

  // ── LA-6e-2: record-response / rubric panel ──────────────────────────
  'recordResp.maxMarks': 'अधिकतम अंक',
  'recordResp.modelAnswer': 'मॉडल उत्तर',
  'recordResp.modelAnswerHint': '(पूरे अंक वाला उत्तर क्या कहता है)',
  'recordResp.modelAnswerPlaceholder':
    'उदा. क्लोरोफिल प्रकाश-संश्लेषण के लिए लाल और नीला प्रकाश सोखता है और हरा परावर्तित करता है।',
  'recordResp.markingPoints': 'अंकन बिंदु',
  'recordResp.markingPointsHint': '(वैकल्पिक — प्रति-बिंदु आंशिक अंक सक्षम करता है)',
  'recordResp.criterionLabelPlaceholder': 'उदा. क्लोरोफिल का नाम लेता है',
  'recordResp.criterionLabelAria': 'मापदंड {n} लेबल',
  'recordResp.criterionMarksAria': 'मापदंड {n} अंक',
  'recordResp.removeCriterionAria': 'मापदंड {n} हटाएँ',
  'recordResp.addMarkingPoint': '+ अंकन बिंदु',
  'recordResp.sumMismatch':
    'अंकन बिंदुओं का योग {sum} है, {max} नहीं — AI प्रति-बिंदु ग्रेड करता है, तो कुल मिलाना ठीक रहेगा।',
  'recordResp.saveError': 'रूब्रिक अभी सहेजा नहीं जा सका। कृपया थोड़ी देर में फिर कोशिश करें।',
  'recordResp.saving': 'सहेजा जा रहा है…',
  'recordResp.updateRubric': 'रूब्रिक अपडेट करें',
  'recordResp.saveRubric': 'रूब्रिक सहेजें',
  'recordResp.noRubricWarning':
    'अभी कोई रूब्रिक नहीं — इसके बिना AI मोटे कीवर्ड मिलान पर निर्भर करता है। निष्पक्ष, पारदर्शी अंकों के लिए एक जोड़ें।',
  'recordResp.addRubric': 'रूब्रिक जोड़ें',
  'recordResp.rubricLabel': 'रूब्रिक:',
  'recordResp.marksLabel': '{count} अंक',
  'recordResp.markingPointsCountOne': '{count} अंकन बिंदु',
  'recordResp.markingPointsCountMany': '{count} अंकन बिंदु',
  'recordResp.edit': 'संपादित करें',
  'recordResp.modelAnswerLabel': 'मॉडल उत्तर:',
  'recordResp.recordForAI': 'AI ग्रेडिंग के लिए उत्तर दर्ज करें',
  'recordResp.class': 'क्लास',
  'recordResp.pickClass': 'एक क्लास चुनें…',
  'recordResp.student': 'विद्यार्थी',
  'recordResp.pickClassFirst': 'पहले एक क्लास चुनें',
  'recordResp.loadingStudents': 'विद्यार्थी लोड हो रहे हैं…',
  'recordResp.pickStudent': 'एक विद्यार्थी चुनें…',
  'recordResp.shortAnswerQuestion': 'लघु-उत्तर प्रश्न',
  'recordResp.loadingQuestions': 'प्रश्न लोड हो रहे हैं…',
  'recordResp.pickQuestion': 'एक प्रश्न चुनें…',
  'recordResp.noShortAnswer':
    'अभी कोई लघु-उत्तर प्रश्न नहीं — पहले प्रश्न बैंक में एक बनाएँ; यहाँ सिर्फ़ लघु-उत्तर प्रश्न ही AI से ग्रेड हो सकते हैं।',
  'recordResp.studentAnswer': 'विद्यार्थी का उत्तर',
  'recordResp.studentAnswerPlaceholder': 'विद्यार्थी का लिखा उत्तर पेस्ट या टाइप करें…',
  'recordResp.queueError': 'यह उत्तर अभी कतार में नहीं डाला जा सका। कृपया थोड़ी देर में फिर कोशिश करें।',
  'recordResp.queuedSuccess':
    'कतार में डाला गया — यह नीचे "AI ग्रेडिंग…" के रूप में दिखता है और कुछ सेकंड में सुझाव में बदल जाता है।',
  'recordResp.aiSuggestsNote': 'AI सुझाव देता है, अंतिम फ़ैसला हमेशा आपका।',
  'recordResp.queuing': 'कतार में डाला जा रहा है…',
  'recordResp.sendToAI': 'AI ग्रेडिंग को भेजें',

  // ── LA-6e-1: edit-safety banner ──────────────────────────────────────
  'editSafety.usedQuestionOne': 'यह प्रश्न 1 असाइनमेंट में इस्तेमाल हो रहा है।',
  'editSafety.usedQuestionMany': 'यह प्रश्न {count} असाइनमेंट में इस्तेमाल हो रहा है।',
  'editSafety.usedSetOne': 'यह समस्या सेट 1 असाइनमेंट में इस्तेमाल हो रहा है।',
  'editSafety.usedSetMany': 'यह समस्या सेट {count} असाइनमेंट में इस्तेमाल हो रहा है।',
  'editSafety.body':
    'आपके बदलाव सिर्फ़ आने वाले असाइनमेंट पर लागू होंगे — मौजूदा असाइनमेंट में विद्यार्थियों को जो दिया गया था वही बना रहेगा।',
  'editSafety.bodyGraded':
    'आपके बदलाव सिर्फ़ आने वाले असाइनमेंट पर लागू होंगे — मौजूदा असाइनमेंट में विद्यार्थियों को जो दिया गया और जिस पर ग्रेडिंग हुई, वही बना रहेगा।',

  // ── LA-6e-1: assignment snapshot preview ─────────────────────────────
  'snapshot.heading': 'विद्यार्थी जो देखते हैं · स्नैपशॉट',
  'snapshot.frozenNote': 'असाइन करते समय फ़्रीज़ किया गया — सेट को बाद में बदलने से यह व्यू नहीं बदलता।',
  'snapshot.hide': 'छिपाएँ',
  'snapshot.show': 'दिखाएँ',
  'snapshot.driftTitle': 'यह असाइनमेंट देने के बाद से लाइव सेट बदल चुका है।',
  'snapshot.driftBody':
    'विद्यार्थी जो देखते हैं और यह असाइनमेंट कैसे ग्रेड होता है, यह ऊपर के फ़्रीज़ स्नैपशॉट से आता है — नए असाइनमेंट अपडेटेड सेट इस्तेमाल करेंगे।',
  'snapshot.driftCompareLink': 'लाइव सेट से तुलना करें',
  'snapshot.updateThis': 'यह असाइनमेंट अपडेट करें…',
  'snapshot.previewBeforeApply': 'कुछ भी लागू करने से पहले हम दिखाएँगे कि क्या बदलेगा।',
  'snapshot.historyNote': 'यह असाइनमेंट अपडेट हुआ था; एक पुराना स्नैपशॉट इतिहास में रखा गया है।',
  'snapshot.undoLast': 'पिछला अपडेट पूर्ववत करें',
  'snapshot.undoing': 'पूर्ववत किया जा रहा है…',
  'snapshot.empty': 'इस असाइनमेंट का स्नैपशॉट खाली है।',

  // ── LA-6e-1: re-sync assignment modal ────────────────────────────────
  'resync.title': 'यह असाइनमेंट नई सामग्री पर अपडेट करें?',
  'resync.subtitle': 'लाइव सेट से दोबारा स्नैपशॉट लेता है। इस पेज से वापस लाया जा सकता है।',
  'resync.loadError': 'प्रीव्यू लोड नहीं हो सका। फिर कोशिश करें।',
  'resync.noDrift': 'स्नैपशॉट पहले से लाइव सेट से मेल खाता है — दोबारा सिंक करने को कुछ नहीं है।',
  'resync.regradeOne':
    '{count} ग्रेड किया गया सबमिशन नई आंसर-की के अनुसार दोबारा ग्रेड होगा। कुछ विद्यार्थियों के अंक बदल सकते हैं।',
  'resync.regradeMany':
    '{count} ग्रेड किए गए सबमिशन नई आंसर-की के अनुसार दोबारा ग्रेड होंगे। कुछ विद्यार्थियों के अंक बदल सकते हैं।',
  'resync.noRegrade': 'कोई ग्रेड किया गया सबमिशन दोबारा ग्रेड नहीं होगा — सिर्फ़ विद्यार्थी जो देखते हैं वही बदल रहा है।',
  'resync.applyError': 'दोबारा सिंक विफल रहा। फिर कोशिश करें।',
  'resync.updating': 'अपडेट हो रहा है…',
  'resync.updateAndRegrade': 'अपडेट करें और {count} दोबारा ग्रेड करें',
  'resync.updateAssignment': 'असाइनमेंट अपडेट करें',
  'resync.diffAddedOne': '{count} प्रश्न जोड़ा गया',
  'resync.diffAddedMany': '{count} प्रश्न जोड़े गए',
  'resync.diffRemovedOne': '{count} प्रश्न हटाया गया',
  'resync.diffRemovedMany': '{count} प्रश्न हटाए गए',
  'resync.diffAnswerOne': '{count} उत्तर बदला गया',
  'resync.diffAnswerMany': '{count} उत्तर बदले गए',
  'resync.diffContentOne': '{count} अन्य सामग्री बदलाव (प्रॉम्प्ट, विकल्प, विजेट)',
  'resync.diffContentMany': '{count} अन्य सामग्री बदलाव (प्रॉम्प्ट, विकल्प, विजेट)',

  // ── LA-6e-1: problem-set preview page ────────────────────────────────
  'psPreview.loadErrorTitle': 'यह प्रीव्यू लोड नहीं हो सका',
  'psPreview.loadErrorDesc': 'हो सकता है यह समस्या सेट हटा दिया गया हो, या आपके पास इसकी पहुँच न हो।',
  'psPreview.backToDashboard': 'डैशबोर्ड पर वापस',
  'psPreview.confirmRemoveAssigned':
    'इस प्रश्न को लाइव सेट से हटाएँ?\n\nमौजूदा असाइनमेंट इसे रखते हैं — वे फ़्रीज़ की गई कॉपी पर ग्रेड और रेंडर करते हैं।',
  'psPreview.confirmRemove': 'इस प्रश्न को सेट से हटाएँ?',
  'psPreview.editingTitle': 'सेट संपादित हो रहा है · लाइव बदलाव',
  'psPreview.readOnlyTitle': 'विद्यार्थी प्रीव्यू · केवल पढ़ने के लिए',
  'psPreview.editingBody':
    'बदलाव सिर्फ़ आने वाले असाइनमेंट पर लागू होंगे — मौजूदा असाइनमेंट में विद्यार्थियों को जो दिया गया था वही बना रहेगा।',
  'psPreview.readOnlyBody':
    'विद्यार्थी सेट को ठीक ऐसे ही देखते हैं — उत्तर छिपे, चर भरे हुए। आप यहाँ टाइप या सबमिट नहीं कर सकते।',
  'psPreview.doneEditing': 'संपादन पूरा',
  'psPreview.editSet': 'सेट संपादित करें',
  'psPreview.back': '← वापस',
  'psPreview.questionsOne': '{count} प्रश्न',
  'psPreview.questionsMany': '{count} प्रश्न',
  'psPreview.emptyTitle': 'इस सेट में अभी कोई प्रश्न नहीं है',
  'psPreview.emptyEditingDesc': 'इसे भरना शुरू करने के लिए नीचे "प्रश्न जोड़ें" बटन इस्तेमाल करें।',
  'psPreview.emptyDesc': 'सेट में प्रश्न जोड़ें, फिर दोबारा प्रीव्यू करें।',
  'psPreview.editQuestion': 'प्रश्न संपादित करें',
  'psPreview.removeFromSet': 'सेट से हटाएँ',
  'psPreview.addMoreTitle': 'और प्रश्न जोड़ें',
  'psPreview.addMoreDesc':
    'प्रश्न बैंक से चुनें, या नया प्रश्न बनाएँ — पूरा होने पर आप यहीं वापस आएँगे।',
  'psPreview.addQuestion': 'प्रश्न जोड़ें',
  'psPreview.viewVersions': 'संस्करण इतिहास देखें',

  // ── LA-6e-4: create / edit question page ─────────────────────────────
  // Widget picker section
  'cqp.widgetLabel': 'इंटरैक्टिव विजेट',
  'cqp.widgetLabelHint': '(वैकल्पिक — सैंडबॉक्स में रेंडर होता है)',
  'cqp.fieldsConfiguredOne': '{count} फ़ील्ड कॉन्फ़िगर',
  'cqp.fieldsConfiguredMany': '{count} फ़ील्ड कॉन्फ़िगर',
  'cqp.edit': 'संपादित करें',
  'cqp.remove': 'हटाएँ',
  'cqp.addWidget': '+ इंटरैक्टिव विजेट जोड़ें',
  // Live preview
  'cqp.previewPlaceholder': 'प्रीव्यू देखने के लिए ऊपर प्रश्न पाठ लिखें...',
  'cqp.preview': 'प्रीव्यू',
  // Variable preview
  'cqp.varPreviewStudentA': 'विद्यार्थी A: {text}',
  'cqp.varPreviewStudentB': 'विद्यार्थी B: {text}',
  // Draft card
  'cqp.draftBadge': 'ड्राफ़्ट',
  'cqp.useThis': 'इसे इस्तेमाल करें',
  'cqp.draftAnswer': 'उत्तर:',
  // AI generation panel
  'cqp.generateWithAI': 'AI से बनाएँ',
  'cqp.generateSubtitle': 'अपने विषय और बाधाओं से प्रश्न ड्राफ़्ट करें',
  'cqp.topicLabel': 'जाँचने का विषय या अवधारणा',
  'cqp.topicPlaceholder': 'जैसे द्विघात बहुपदों का गुणनखंडन, ऊष्मागतिकी के नियम…',
  'cqp.typeLabelShort': 'प्रकार',
  'cqp.countLabel': 'संख्या',
  'cqp.typeMcq': 'MCQ',
  'cqp.typeNumeric': 'संख्यात्मक',
  'cqp.typeFillBlank': 'रिक्त स्थान',
  'cqp.typeMultiSelect': 'बहु-चयन',
  'cqp.generating': 'बनाया जा रहा है…',
  'cqp.generate': '✨ बनाएँ',
  'cqp.generationFailed': 'बनाना विफल रहा।',
  'cqp.retry': 'फिर कोशिश करें',
  'cqp.aiUnavailable':
    'AI प्रश्न-निर्माण अभी उपलब्ध नहीं है। कृपया थोड़ी देर बाद कोशिश करें, या नीचे अपना प्रश्न खुद लिखें।',
  'cqp.draftsCountOne': '{count} ड्राफ़्ट — फ़ॉर्म भरने के लिए "इसे इस्तेमाल करें" पर क्लिक करें',
  'cqp.draftsCountMany': '{count} ड्राफ़्ट — फ़ॉर्म भरने के लिए "इसे इस्तेमाल करें" पर क्लिक करें',
  // Page header
  'cqp.titleEdit': 'प्रश्न संपादित करें',
  'cqp.titleCreate': 'प्रश्न बनाएँ',
  'cqp.subtitleEdit': 'साझा प्रश्न बैंक में इस प्रश्न को अपडेट करें।',
  'cqp.subtitleCreate': 'साझा प्रश्न बैंक में एक प्रश्न जोड़ें।',
  'cqp.back': '← वापस',
  // Success state
  'cqp.successUpdated': 'प्रश्न अपडेट हुआ!',
  'cqp.successCreated': 'प्रश्न बन गया!',
  'cqp.successBodyUpdated': 'प्रश्न #{id} प्रश्न बैंक में अपडेट कर दिया गया है।',
  'cqp.successBodyCreated': 'प्रश्न #{id} प्रश्न बैंक में जोड़ दिया गया है।',
  'cqp.createAnother': 'एक और बनाएँ',
  'cqp.backToBank': 'प्रश्न बैंक पर वापस',
  // Chapter card
  'cqp.chapter': 'अध्याय',
  'cqp.subject': 'विषय',
  'cqp.selectSubject': 'विषय चुनें...',
  'cqp.selectChapter': 'अध्याय चुनें...',
  'cqp.chapterOption': '{name} (कक्षा {std})',
  'cqp.difficulty': 'कठिनाई',
  'cqp.difficultyHint': '1 = सबसे आसान, 5 = सबसे कठिन',
  // Subpart tabs
  'cqp.partTab': 'भाग {letter}',
  'cqp.addPart': '+ भाग जोड़ें',
  // Question fields
  'cqp.questionType': 'प्रश्न का प्रकार',
  'cqp.typeMcqLong': 'बहुविकल्पीय (MCQ)',
  'cqp.typeFillBlankLong': 'रिक्त स्थान भरें',
  'cqp.questionText': 'प्रश्न पाठ',
  'cqp.latexHint': '(LaTeX: $x^2$ या $$\\frac{a}{b}$$)',
  'cqp.variableTokenHint': ' · चर टोकन के लिए {{a}} का उपयोग करें',
  'cqp.questionTextPlaceholderVar': 'जैसे हल करें ${{a}}x + {{b}} = {{c}}$',
  'cqp.questionTextPlaceholder': 'जैसे हल करें $x^2 - 4 = 0$',
  'cqp.imageUrl': 'इमेज URL',
  'cqp.imageUrlHint': '(वैकल्पिक — प्रश्न पाठ के ऊपर दिखता है)',
  'cqp.imageUrlPlaceholder': 'इमेज URL पेस्ट करें या फ़ाइल अपलोड करें',
  'cqp.uploading': 'अपलोड हो रहा है...',
  'cqp.uploadImage': 'इमेज अपलोड करें',
  'cqp.imageFormats': '5 MB से कम का PNG, JPG, GIF या WebP।',
  'cqp.imageUploadFailed': 'इमेज अपलोड विफल रहा। 5 MB से कम का PNG, JPG, GIF या WebP आज़माएँ।',
  'cqp.imagePreviewAlt': 'प्रीव्यू',
  'cqp.solution': 'विस्तृत हल',
  'cqp.solutionHint': '(वैकल्पिक — ग्रेडिंग के बाद विद्यार्थियों को दिखता है)',
  'cqp.solutionPlaceholder': 'चरण-दर-चरण हल। $...$ के साथ LaTeX समर्थित।',
  'cqp.hint': 'संकेत',
  'cqp.hintHint': '(वैकल्पिक — अभ्यास के दौरान जूझ रहे विद्यार्थियों को दिखता है)',
  'cqp.hintPlaceholder': 'सही दिशा में एक इशारा।',
  // Variable constraints
  'cqp.varConstraints': 'चर बाधाएँ',
  'cqp.varConstraintsHint': '— अपने प्रश्न के हर {{token}} के लिए सीमा तय करें',
  'cqp.min': 'न्यून',
  'cqp.max': 'अधि',
  'cqp.integer': 'पूर्णांक',
  // MCQ options + answer
  'cqp.options': 'विकल्प',
  'cqp.optionPlaceholder': 'विकल्प {key}',
  'cqp.correctAnswer': 'सही उत्तर',
  'cqp.correctAnswerTokenHint': '({{tokens}} का उपयोग कर सकते हैं, जैसे ({{c}} - {{b}}) / {{a}})',
  'cqp.selectCorrectOption': 'सही विकल्प चुनें...',
  'cqp.correctNumericPlaceholder': 'जैसे 42',
  'cqp.correctAnswerPlaceholder': 'सही उत्तर',
  'cqp.removePart': 'यह भाग हटाएँ',
  // Submit
  'cqp.updateQuestion': 'प्रश्न अपडेट करें',
  'cqp.saveQuestion': 'प्रश्न सहेजें',
  'cqp.cannotSubmitHint': 'सहेजने के लिए एक अध्याय चुनें और सभी प्रश्न पाठ भरें।',
  'cqp.saveFailed': 'सहेजना विफल रहा। कृपया फिर कोशिश करें।',

  // ── LA-6e-3: problem-set version history ─────────────────────────────
  'psVersions.loadErrorTitle': 'संस्करण इतिहास लोड नहीं हो सका',
  'psVersions.loadErrorDesc': 'हो सकता है यह सेट हटा दिया गया हो, या आपके पास पहुँच न हो।',
  'psVersions.backToDashboard': 'डैशबोर्ड पर वापस',
  'psVersions.title': 'संस्करण इतिहास',
  'psVersions.description':
    'इस समस्या सेट का हर प्रकाशित सामग्री संस्करण। नए असाइनमेंट नए संस्करण पिन करते हैं; मौजूदा असाइनमेंट उसी संस्करण पर रहते हैं जो उन्हें दिया गया था।',
  'psVersions.backToPreview': '← प्रीव्यू पर वापस',
  'psVersions.emptyTitle': 'अभी कोई संस्करण नहीं',
  'psVersions.emptyDesc': 'जब यह सेट पहली बार असाइन या दोबारा सिंक होता है, तब एक संस्करण बनता है।',
  'psVersions.version': 'v{number}',
  'psVersions.target': 'लक्ष्य',
  'psVersions.against': 'तुलना',
  'psVersions.questionsCountOne': '{count} प्रश्न',
  'psVersions.questionsCountMany': '{count} प्रश्न',
  'psVersions.pinnedOne': '{count} पिन किया असाइनमेंट',
  'psVersions.pinnedMany': '{count} पिन किए असाइनमेंट',
  'psVersions.diffStart': 'डिफ़ शुरू करने के लिए एक संस्करण पर क्लिक करें।',
  'psVersions.diffSecond': 'तुलना करने के लिए दूसरे संस्करण पर क्लिक करें।',
  'psVersions.diff': 'डिफ़',
  'psVersions.clearSelection': 'चयन हटाएँ',
  'psVersions.diffLoadError': 'डिफ़ लोड नहीं हो सका। फिर कोशिश करें।',
  'psVersions.diffAddedOne': '{count} प्रश्न जोड़ा गया',
  'psVersions.diffAddedMany': '{count} प्रश्न जोड़े गए',
  'psVersions.diffRemovedOne': '{count} प्रश्न हटाया गया',
  'psVersions.diffRemovedMany': '{count} प्रश्न हटाए गए',
  'psVersions.diffAnswerOne': '{count} उत्तर बदला गया',
  'psVersions.diffAnswerMany': '{count} उत्तर बदले गए',
  'psVersions.diffContentOne': '{count} सतही सामग्री बदलाव',
  'psVersions.diffContentMany': '{count} सतही सामग्री बदलाव',
  'psVersions.comparing': '{target} की {against} से तुलना।',
  'psVersions.initialVersion': 'कुछ नहीं (प्रारंभिक संस्करण)',
  'psVersions.identical': 'सामग्री एक जैसी है।',

  // ── LA-6e-3: classroom join-code widget ──────────────────────────────
  'classCode.heading': 'क्लासरूम जॉइन कोड',
  'classCode.noExpiry': 'कोई समय-सीमा नहीं',
  'classCode.expired': 'समय समाप्त',
  'classCode.expiresLtHour': '<1 घंटे में समाप्त',
  'classCode.expiresHours': '{hours} घंटे में समाप्त',
  'classCode.expiresDays': '{days} दिन में समाप्त',
  'classCode.copied': 'कॉपी हो गया!',
  'classCode.copyCode': 'कोड कॉपी करें',
  'classCode.copyLink': 'लिंक कॉपी करें',
  'classCode.regenConfirmAria': 'जॉइन कोड फिर बनाने की पुष्टि करें',
  'classCode.regenWarning':
    'फिर बनाने पर मौजूदा कोड अमान्य हो जाता है। जो विद्यार्थी अभी तक नहीं जुड़े, उन्हें नया कोड चाहिए होगा।',
  'classCode.yesRegenerate': 'हाँ, फिर बनाएँ',
  'classCode.regenerate': 'फिर बनाएँ',
  'classCode.regenerateTitle': '{classroom} के लिए जॉइन कोड फिर बनाएँ',
  'classCode.generating': 'बनाया जा रहा है…',
  'classCode.generate': 'जॉइन कोड बनाएँ',

  // ── Streak badge ─────────────────────────────────────────────────────
  'streak.days': '{count} दिन की स्ट्रीक',
  'streak.tierStarter': 'जोश में',
  'streak.tierWeek': 'हफ़्ते के हीरो',
  'streak.tierMonth': 'महीने के मास्टर',
  'streak.tierChampion': 'चैंपियन',
  'streak.best': 'सर्वश्रेष्ठ: {count}',
  'streak.grace': 'ग्रेस ✓',
  'streak.graceTitle': 'ग्रेस दिन इस्तेमाल हुआ — एक दिन छूटने पर भी स्ट्रीक बनी रही',

  // ── Connectivity (PWA offline indicator) ─────────────────────────────
  'connectivity.offlineTitle': 'आप ऑफ़लाइन हैं',
  'connectivity.offlineBanner': 'आप ऑफ़लाइन हैं — सहेजा हुआ काम दिखाया जा रहा है।',
  'connectivity.backOnline': 'फिर से ऑनलाइन।',

  // ── Sync (offline write queue status — MSO-8) ────────────────────────
  'sync.savedOffline': 'इस डिवाइस पर सहेजा गया · ऑनलाइन होने पर सिंक होगा',
  'sync.syncing': 'सिंक हो रहा है…',
  'sync.synced': 'सहेजा गया',
  'sync.syncFailed': 'सिंक नहीं हो सका — फिर कोशिश होगी',
  'sync.pendingOne': '{count} बदलाव सिंक होना बाकी',
  'sync.pendingMany': '{count} बदलाव सिंक होने बाकी',

  // ── PWA (install to home screen) ─────────────────────────────────────
  'pwa.installPrompt': 'OpenShiksha को अपनी होम स्क्रीन पर जोड़ें।',
  'pwa.install': 'इंस्टॉल करें',
  'pwa.installDismiss': 'इंस्टॉल सूचना बंद करें',
  // ── PWA (service worker update prompt) ───────────────────────────────
  'pwa.updateAvailable': 'नया वर्शन उपलब्ध है।',
  'pwa.refresh': 'रिफ़्रेश करें',
  // ── Web Push notifications (due-date reminders) ──────────────────────
  'push.prompt': 'अपने फ़ोन पर रिमाइंडर पाएँ।',
  'push.enable': 'चालू करें',
  'push.dismiss': 'सूचना अनुरोध बंद करें',
  'push.enabled': 'रिमाइंडर चालू',
  'push.label': 'पुश सूचनाएँ',
  'push.description': 'असाइनमेंट जल्द देय होने पर इस डिवाइस पर सूचना पाएँ।',
  'push.blocked': 'आपके ब्राउज़र सेटिंग्स में सूचनाएँ अवरुद्ध हैं।',
};
