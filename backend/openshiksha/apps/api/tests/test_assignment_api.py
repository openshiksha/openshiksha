...
from openshiksha.apps.core.models import (
    User, UserRole, Board, School, Standard, Subject, Chapter, ClassRoom,
    Question, SubjectRoom, ProblemSet, Assignment, Submission,
)
...
def teacher(db, school):
    return User.objects.create_user(
        username='teacher_api', password='pass', role=UserRole.TEACHER, school=school
    )
...
def student(db, school):
    return User.objects.create_user(
        username='student_api', password='pass', role=UserRole.STUDENT, school=school
    )
...
def other_student(db, school):
    return User.objects.create_user(
        username='other_student_api', password='pass', role=UserRole.STUDENT, school=school
    )
...
def problem_set(db, school, standard, subject, chapter, question):
    ps = ProblemSet.objects.create(
        ...
        title='Kinematics Practice #1', number=1,
    )
...
class TestSubmissionWorkflow:
    ...
    def test_student_can_update_submission(self, api_client, student, assignment):
        sub = Submission.objects.create(
            assignment=assignment, student=student, answers={'1': 1}, completion=0.5
        )
        api_client.force_authenticate(user=student)
