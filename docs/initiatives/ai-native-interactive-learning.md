...
| DTB-5b | **Surface randomization in the Describe-it UI**: an "each student gets different numbers" toggle on the prompt box that sets `allow_variables` and persists the returned `variable_constraints` on attach, so the wow is on-screen. | ✅ Done (#TBD) |
