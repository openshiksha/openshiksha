...
**Last updated:** 2026-06-29 (latest) — **NEW TOP INITIATIVE PROMOTED:
...
|:--:|---|---|---|---|
| 1 | [Observability Activation](2026-observability-activation.md) | **Active** | **Promoted 2026-06-29.** Turns the validated-but-dead Production Observability artifacts into deployable, CI-gated k8s manifests. **Batch 1 (OACT-1..5) planned** in [2026-06-29-plan.md](../daily-plans/2026-06-29-plan.md): a self-contained `k8s/monitoring/` kustomization (Prometheus + ALT-1 rules → Alertmanager + `alert_to_ntfy.py` sidecar → Grafana provisioned with the MET-5 dashboard), all gated by kustomize + kubeconform `-strict` and **not** wired into the auto-deploying prod overlay (operator applies with one `kubectl apply -k k8s/monitoring`); plus wiring `record_backup_run` into the nightly backup CronJob so `openshiksha_backup_age_seconds` reads real data in prod. The third named follow-up (`VITE_SENTRY_DSN` in CI) is already done (`ci-cd.yaml:530`). All additive — qa/dev kustomize byte-for-byte unchanged, monitoring stack ignored by the prod overlay. | **Build OACT-1..5** lowest-risk-first (OACT-1 → 2 → 3 → 4 → 5); floor-value subset OACT-1 + OACT-4 + OACT-5. After: operator applies `k8s/monitoring/` on the droplet. |
