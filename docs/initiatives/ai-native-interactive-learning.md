...
| DTB-4 | **Demo golden-path beat + capture**: extend `docs/demo/golden-path.md` (Beat 2 text landed with DTB-3); **capture a screenshot/gif** on a dev-stack run; an e2e smoke that drives type → generate → render → (student) grade. First use of the "routine accumulates the demo" mechanic. | ✅ Done (#TBD) |
