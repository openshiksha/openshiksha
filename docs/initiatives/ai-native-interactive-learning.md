...
| GSV-2 | **Step-solver widget** (`step-solver`): a `function-plotter`-style sandboxed widget — student enters successive lines; the host calls GSV-1 per line and shows ✓/✗ liveness. Answer-producing → reports the final line through the existing per-subpart grader (AI nowhere near the grade). Schema + parity-guarded vendored schema (DTB-1 path). | ⬜ Not started |
