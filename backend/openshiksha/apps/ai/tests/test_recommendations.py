...
def db_setup(db):
    ...
    from openshiksha.apps.core.models import (
        Board, Chapter, ClassRoom, School, Standard, Subject, SubjectRoom, User
    )
    ...
    subject_room = SubjectRoom.objects.create(
...
def _make_resolved_gap(db_setup, chapter, avg_score, days_ago=2):
    ...
    LearningGap.objects.filter(pk=gap.pk).update(
        refreshed_at=timezone.now() - timedelta(days=days_ago)
    )
    return gap
...
class TestContentRecommendationHelpers:
    def test_priority_for_severe_gap(self):
        assert (
            ContentRecommendation.priority_for_reason(RecommendationReason.SEVERE_GAP)
            == RecommendationPriority.URGENT
        )

    def test_priority_for_moderate_gap(self):
        assert (
            ContentRecommendation.priority_for_reason(RecommendationReason.MODERATE_GAP)
            == RecommendationPriority.HIGH
        )

    def test_priority_for_spaced_review(self):
        assert (
            ContentRecommendation.priority_for_reason(RecommendationReason.SPACED_REVIEW)
            == RecommendationPriority.HIGH
        )

    def test_priority_for_mild_gap(self):
        assert (
            ContentRecommendation.priority_for_reason(RecommendationReason.MILD_GAP)
            == RecommendationPriority.MEDIUM
        )

    def test_priority_for_next_topic(self):
        assert (
            ContentRecommendation.priority_for_reason(RecommendationReason.NEXT_TOPIC)
...
class TestGenerateRecommendations:
    def test_no_gaps_no_ticks_returns_next_topic_suggestions(self, db_setup):
        ...
        recs = generate_recommendations_for_student(
            db_setup['student'], db_setup['subject_room']
        )
